...
import { criarVendedoraAction } from "./actions";

export function VendedoraForm() {
  const ref = useRef<HTMLFormElement>(null);
  return (
    <form
      ref={ref}
      action={async (fd) => {
        await criarVendedoraAction(fd);
        ref.current?.reset();
      }}
      className="flex flex-col gap-5 max-w-md"
    >
      <Field id="vend-nome" name="nome" label="Nome" type="text" autoComplete="name" />
      <Field id="vend-email" name="email" label="E-mail" type="email" autoComplete="off" />
      <Field
        id="vend-senha"
        name="senha"
...
function Submit() {
  ...
  return (
    <button
      ...
      "
    >
      {pending ? "Cadastrando…" : "Cadastrar vendedora"}
    </button>
  );
}
