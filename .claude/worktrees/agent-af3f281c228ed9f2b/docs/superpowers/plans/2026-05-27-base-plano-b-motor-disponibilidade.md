...
## Status: A IMPLEMENTAR (2026-05-27)
...
2. **Manutenção = `[retiradaDataReal, devolucaoDataReal)`.** O spec não define quais campos delimitam um bloqueio de manutenção; reutilizamos os dois campos de data reais já existentes em `BloqueioVestido` como o período fora de serviço. Meio-aberto como as demais janelas (Grill 1): `devolucaoDataReal` é o primeiro dia de volta ao serviço. Gera **uma** janela do tipo `manutencao`. **⚠️ Decisão a confirmar com o cliente** (registrar no spec se confirmada).
...
5. **Datas inválidas = erro explícito.** `calcularJanelas` lança se faltar `casamentoData` numa reserva, ou `retiradaDataReal`/`devolucaoDataReal` numa manutenção — são invariantes de integridade que o CRUD (plano futuro) garante.
...
- [ ] **Step 1: Criar os tipos de domínio em `tipos.ts`**
...
- [ ] **Step 2: Escrever o teste que falha para os helpers de data**
...
- [ ] **Step 1: Escrever os testes que falham para `calcularJanelas`**
...
- [ ] **Step 3: Implementar `calcularJanelas` em `motor.ts`**
...
- [ ] **Step 1: Adicionar os testes que falham para `vestidoDisponivel`**
...
- [ ] **Step 3: Implementar `vestidoDisponivel` em `motor.ts`**
...
- [ ] **Step 1: Criar o barrel `index.ts`**
...
- [ ] **Step 2: Conferir que o import público resolve via alias `@`**
...
- [ ] **Step 4: Conferir tipos (sem `any` escondido)**
