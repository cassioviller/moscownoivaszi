...
2. ~~**S-A2 🟡** — pedir à dona as fotos que faltam~~ — **fechada por decisão em
   16/08/2026: não há mais fotos.** As 136 saídas ficam como piso.
...
| ~~S-A10~~ | ~~**"Duração da prova" é a única linha do bloco de disponibilidade sem contrapartida editável.** Para mudar, só `PATCH` na API. E o cabeçalho do próprio arquivo (`configuracoes/index.tsx:22-25`, do E98) afirma que "isso mora em 'Cabines & horário', dentro de Atendimentos" — **não mora**: `atendimentos/config.tsx` não expõe o campo, e o `EditarEm` do card (`:173`) leva a uma tela sem ele.~~ **FECHADA em `f4cb527`** (agente B2), nas duas pontas: o campo entrou como Select de 30/60/90/120 **minutos** no mesmo form e mesmo PATCH da tela para onde o `EditarEm` aponta, e o cabeçalho diz a verdade. A unidade (slots de 30 min — o cuidado da S-A7) virou régua única `lib/duracao-da-prova.ts` sobre o `SLOT_MINUTOS` do core, absorvendo a conta inline da tela de configurações (regra 26). | 🟡 | execução E148 |
...
| ~~S-A16~~ | ~~**A lavagem não entra na régua do estoque.** A janela do E154 é a de USO, como a spec pediu; mas o saiote também vai à lavagem, e a régua da loja reserva 7 dias para ela no vestido (P1: *"uma semana, lavagem externa"*). A conta é **otimista**: saiote devolvido no dia 21 aparece livre no 22, quando está molhado. Como o épico avisa e não bloqueia, o custo é um aviso que deixa de aparecer — não uma venda recusada à toa. Se a peça de estoque tem ciclo de lavagem é pergunta de produto. **DECIDIDO em 2026-08-07** (folha, respondida na sessão): **a lavagem do estoque vira CONFIGURÁVEL nas configurações** — um campo próprio por loja (irmão do `lavagemDiasDepois` do vestido), nascendo em 0 para que nada mude até a dona preencher, somado à janela de uso no cálculo do estoque (`estoque.ts`, que hoje deixa a lavagem de fora de propósito). Deixou de ser pergunta; é sobra de código com a decisão escrita — e toca schema/seed, então a régua do banco virgem roda antes do commit.~~ **FECHADA em `8179ae5`.** `estoque_lavagem_dias_depois` nasce em `regra_disponibilidade` com default 0 (o comportamento de sempre — nada muda até a loja preencher), atravessa openapi/codegen/seed, ganha campo em "Cabines & horário" e linha de leitura em Configurações. O fim da janela do estoque soma a lavagem: com 7 dias, o exemplo da sobra fecha — o saiote e o vestido do mesmo contrato voltam juntos (21/09 → 28/09, pregado em teste com vermelho antes). Banco-virgem passou inteira; migração aplicada no dev. | ✅ | execução E154 · decisão 2026-08-07 · `8179ae5` |
| ~~S-A17~~ | ~~**A fila da costureira não tem tela própria por trabalho.** O E155 põe confecção e ajuste na mesma lista e o item do orçamento aponta o trabalho, mas o link do item leva à FILA (`/ajustes?recorte=todos`), não ao trabalho — não existe rota `/ajustes/:id`. Numa loja com fila longa, "na fila da costureira" obriga a procurar a olho. Enquanto a confecção era inexistente isso não pesava; agora que ela tem custo e é cobrada, pesa.~~ **FECHADA em `8b9c574`**: a ficha `/ajustes/:id` nasce SEM rota nova de servidor (mesma lista da fila, mesma query key, mesmo cache) com status, selo, custo, prazo, origem e as ações da fila unificadas em `useAcoesDeAjuste`; os links do item de orçamento e da origem da peça apontam o trabalho. ~~Falta um spec E2E para a rota nova — anotado no registro da sessão.~~ **A dívida foi paga em `948b97d`** (2026-08-07): `e2e/60-ficha-do-trabalho.spec.ts` prega os três caminhos — deep link, marcar feito sobrevivendo ao recarregar, item de orçamento → ficha — com fixture que se apaga (resíduo zero medido). E2E 161 → 164. | 🔵 | execução E155 · `948b97d` |
| ~~S-A18~~ | ~~**A ausência não olha o que já está marcado.** Registrar férias por cima de uma agenda cheia é aceito em silêncio: o E151 decidiu (com a spec) que ela só impede o NOVO, mas quem cadastra não fica sabendo que há atendimentos naquele intervalo. Um aviso na hora de marcar — *"há 4 atendimentos nesse período; eles não serão alterados"* — fecharia o buraco entre a decisão certa e a pessoa que precisa agir sobre ela. Remarcação em lote segue sendo decisão de produto; **contar e avisar não é**.~~ **FECHADA por DECISÃO em 2026-08-07** (folha de perguntas, respondida na sessão): **férias por cima de agenda cheia entram em silêncio, de propósito** — a ausência já barra agendamento novo e não mexe no que estava marcado; é assim que deve ser, e agora está escrito. | ✅ | execução E151 · decisão 2026-08-07 |
...
| ~~S-A24~~ | ~~**A dona nomeou uma distinção que o sistema não sabe dizer.** Ao responder a P8 ela não disse "domingo é dia de trabalho": disse *"domingo com hora marcada"* — atende quando a noiva pede, e não é expediente de rotina. O modelo tem uma alavanca só (`diasFuncionamento`: o dia está aberto ou o `POST` recusa), então a resposta virou **domingo aberto**, que é o que não perde a venda — mas a grade passa a oferecer domingo como qualquer outro dia, e ninguém marcou para trabalhar de rotina. Um dia "sob demanda" — fora da grade, mas aceito quando alguém insiste — não existe no modelo. É pergunta de produto antes de ser código, e o custo de hoje é pequeno: uma grade que oferece a mais.~~ **FECHADA por DECISÃO em 2026-08-07** (folha de perguntas, respondida na sessão): **domingo mostra os horários livres para a noiva escolher** — aberto é o que não perde a venda, e a resposta anterior da dona ('domingo com hora marcada') fica atendida pelo aberto até que um estado 'sob demanda' seja desenhado como épico próprio, se um dia for. | ✅ | decisão P8, sessão 5 · decisão 2026-08-07 |
...
| ~~S-A27~~ | ~~**Classificar as 496 peças do acervo com o "Tipo de peça" que existe desde julho.** Nasce da resposta da S-A3 (mesmo caminho comercial): o vocabulário está pronto (Noiva, Festa, Dama, Madrinha, Debutante, Acessório — `configuracao-inicial.ts:206`) e o uso real é zero. É trabalho de catalogação com a dona dizendo o que é o quê — uma tarde, não código. Enquanto não acontece, o filtro por linha de negócio (E149/`bf1162e`) filtra o vazio.~~ | 🟡 | resposta da S-A3 · decisão 2026-08-07 — **classificadas em 2026-08-16** (decisão da dona: "classifica"), pela regra da própria S-A3: a linha de noiva é indexada por nome de modelo, e os 132 códigos L001–L132 são isso; bolero, mantilha e saiote/crinol são Acessório pelo nome. `docs/migracoes/2026-08-16-s-a27-tipo-de-peca-do-legado.sql`, aplicado no `moscow_base`: **126 Noiva · 5 Acessório · 1 pendente** (L084 "Solussaia + Manga", que o nome não decide — fica para a dona), idempotente na segunda passada (INSERT 0). A sobra dizia 496 peças; o `moscow_base` tem 132 |
