...
| S-A25 | **`DELETE /vestidos/:id` apaga peça do acervo sem guarda nenhuma.** `vestidos.ts:510-514`, cinco linhas: não conta contrato, não conta orçamento, não conta bloqueio, não oferece 409 e não faz soft-delete. Com `contrato_itens.vestidoId` em `set null` (S-A14), apagar uma peça vendida transforma o item do contrato em descrição livre — em silêncio, e a guarda do E150 deixa de valer para aquele contrato numa reedição. **A assimetria é o achado:** a migração de faxina da S-A13, escrita hoje, tem guarda explícita contra exatamente isso (`AND NOT EXISTS … contrato_itens`), e a rota que a loja usa todo dia não tem. Hoje o estrago não é observável (`contrato_itens` tem 0 linhas no banco de dev), e é isso que o torna fácil de deixar passar. Nasceu da [conferência de 2026-08-05](../2026-08-05-conferencia-de-sobras.md). | 🟠 | S-A14 · [conferência de 2026-08-05](../2026-08-05-conferencia-de-sobras.md) |
