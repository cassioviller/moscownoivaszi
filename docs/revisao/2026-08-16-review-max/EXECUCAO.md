...
| ~~**E262**~~ | ~~a aspa reta sai das frases que uma pessoa lê~~ | S-RM19 🟠, S-RM20 🔵, S-RM21 🔵, S-RM22 🔵 | ✅ `bb4a8606` · [relatório](execucao/E262.md) — **o critério que a S-RM22 pedia está escrito: valor DIGITADO perde a aspa; rótulo FIXO citado dentro de uma frase mantém a aspa, e ela é CURVA.** As duas subfamílias eram maiores do que o plano mediu — A tem **6 sítios e 8 aspas** (dois em texto JSX nu, invisíveis às buscas por `\"` e por `"${`) e B tem **4**, sendo o quarto a mesma frase repetida na tela. A régua nova achou um **sétimo sítio que sobra nenhuma nomeia**. **Nenhuma citação de manual quebrou** — o ponto de encontro com o E263 se resolveu sozinho. Frontend 1053 (117) → **1078 (120)** medido no `main` com os quatro dentro |
...
**A linha da integração é `dcd6b608`**: o `e2e/15-onda5-pdf-e-folha.spec.ts`
volta ao gesto humano — preencher De e Até em sequência —, que estivera
proibido por comentário citando a S-RM17. **A guarda foi medida antes de ser
afirmada**, revertendo o conserto só naquela tela: reprova em **3 de 4
execuções**, sempre na asserção do campo "Até" e sempre ANTES do clique
(`Expected: "2024-04-04" · Received: "2026-08-01"` — a janela que carimbou os
302). O verde da quarta é a corrida não acontecendo. Ela nunca reprova por
engano, e **a prova determinística do fecho é o teste de unidade do E261**, não
esta — está dito na letra da cena.

**Régua do fecho, as cinco medidas em série no `main` em 17/08: API 1905 (272
arquivos, 12,2 min) · frontend 1078 (120 arquivos, 36,1 s) · E2E 188 em 8,0
min, 0 skipped · banco virgem inteiro (16, ~1 min) · typecheck verde em 5
projetos.** O frontend saiu de 1053 (117): **+25 testes e +3 arquivos**, e de
novo **não é a soma dos relatórios** — cada agente mediu no worktree dele, sem
enxergar os outros (E261: 1070/118 · E262: 1056/118 · E263: 1054/117 · E264:
1057/118). A recontagem depois da integração é que vale (regra 36). O E2E
ficou em **188**: nenhum teste novo, e a cena do E260 mudou de gesto sem mudar
de contagem.

**O ponto de encontro dos manuais NÃO colidiu.** O plano previu que o E262
poderia quebrar citação que o E263 mantém, e nomeou a regra de desempate (a
tela é a verdade, o manual a segue). A `varredura-manuais-textos` está entre os
120 verdes: **nenhuma citação quebrou**, e a razão é que os dois consertos
pediram coisas opostas na mesma família — o E262 tirou aspa de valor digitado
(que os manuais não citam) e manteve a do rótulo fixo (que é o que eles citam).

**E um vermelho de agente NÃO era do repositório.** O E262 mediu a suíte de API
vermelha na base em dois arquivos (`expected 200 "OK", got 410 "Gone"` no
`res.download`) e provou que era anterior ao épico dele, deixando a pergunta
para a régua do orquestrador. **Medido no `main` fora de worktree: os dois
passam — 2 arquivos, 11 testes —, e a suíte inteira deu 1905/272 verde.** É
ambiente, não defeito, e fica registrado como S-RM38 🔵 com as duas medições
...
forte disso até agora — **oito fechadas, quinze abertas**, e quatro delas
achadas por réguas que os próprios épicos escreveram (o sétimo sítio da
S-RM20, o `recepcao.html:332` do E263, o piso `>=` da S-RM33, e o oitavo
arquivo da S-RM18). **A pergunta que fica para o próximo plano é outra: não
"quantas vão abrir", mas "quando é hora de parar de construir régua".**
...
| ~~S-RM19~~ | 🟠 | `catalogo/[atributoId]/editar.tsx:145` e `:180` | ~~**O catálogo passa a RECUSAR sem aspas e a CONFIRMAR com elas** — e é consequência direta da decisão da S-RM16, executada com a medição estreita. Os dois toasts da mesma tela escrevem `` `"${atributo.nome}" saiu do catálogo.` `` e `` `"${opcaoParaApagar.valor}" saiu de "${atributo.nome}".` `` (o segundo com DUAS). Quem apaga uma opção lê *Marfim classifica 3 peça(s)…* na recusa e *"Marfim" saiu de "Cor".* na confirmação, **na mesma sessão, a dois cliques de distância**. A decisão foi tomada sobre *"as únicas duas em frase que uma pessoa lê"*, e **são seis**. O E259 não tocou porque `src/pages` era do E258~~ | ✅ `bb4a8606` (E262) — **e ela era PIOR do que a sobra diz.** A sobra descreve a incoerência entre a RECUSA e a CONFIRMAÇÃO; medido o arquivo inteiro, a mesma tela também **PERGUNTA** com aspas (`editar.tsx:404` e `:421`, texto JSX NU — a aspa não está escapada, não está em template literal e não encosta em `${`, então nenhuma das medições anteriores podia vê-la). Quem apaga uma opção lê **três frases sobre a mesma palavra, sem sair do lugar**: `Apagar a opção "Marfim"?` com aspas · `Marfim classifica 3 peça(s)…` sem · `"Marfim" saiu de "Cor".` com duas. São **seis sítios e oito aspas**, não quatro |
| ~~S-RM20~~ | 🔵 | `lib/documento-na-porta.ts:41` · `admin/perfis.tsx:81` | ~~As outras duas da mesma família: `` `${tipo} "${valor.trim()}" não é um ${tipo} válido: …` `` e `` `"${nome}" nasce sem acesso nenhum — marque na matriz o que ele pode.` ``. **As duas são frase que uma pessoa lê, e nenhuma é protocolo** — estão fora da decisão só porque a medição que a sustentou contava duas onde havia seis~~ | ✅ `bb4a8606` (E262) — as duas consertadas pelo mesmo critério, e a régua achou um **sétimo sítio que sobra nenhuma nomeia**: `reservas/[bloqueioId].tsx:2041`, em texto JSX nu. **A varredura exploratória do épico não o achou e a RÉGUA achou** — porque a varredura exigia que a aspa viesse depois de prosa e ali ela abre a linha. É a regra 34 pagando na mesma sessão em que foi cumprida |
| ~~S-RM21~~ | 🔵 | `financeiro/folha.tsx:524` × `proprietario.html:615` | ~~**A mesma classe da S-RM16 pelo avesso**: a TELA cita um rótulo entre aspas RETAS (*"…o envio é aqui embaixo, em "Fechar com a contabilidade"."*) e o manual o italiciza, sem elas. O molde teve de parar antes da aspa por causa disso — o pedaço declarado é `— o envio é aqui embaixo, em`, e o rabo da frase fica fora da régua~~ | ✅ `bb4a8606` (E262) — a tela passa a citar o rótulo com aspa CURVA. **E o pedaço declarado NÃO cresce**, como a S-RM16 também não cresceu, mas pelo motivo OPOSTO ao previsto: ele fica em 28 caracteres, e o caminho para 58 é o MANUAL passar a usar a aspa curva — medido aqui, e é território do E263 (fica como S-RM35) |
| ~~S-RM22~~ | 🔵 | `routes/catalogo.ts:110` | ~~**Duas linhas abaixo da que perdeu as aspas, elas ficaram**: a continuação da mesma mensagem diz `desmarque \"Atributo ativo\"` — aspa reta em volta de um rótulo FIXO, que é caso diferente do da interpolação e não estava na decisão. Fica registrada para que a próxima passada **decida a família inteira de uma vez**, em vez de uma linha por épico~~ | ✅ `bb4a8606` (E262) — **a família foi decidida de uma vez, que é o que a sobra pedia, e ela tinha QUATRO sítios, não um.** O critério escrito: **valor DIGITADO perde a aspa; rótulo FIXO citado dentro de uma frase mantém a aspa, e ela é CURVA** — mesma razão nas duas metades, pedidos opostos, porque `Com Devolvi o valor marcado abaixo` deixa de ser português. A curva já era a grafia de **10 sítios de produção**, um deles (`equipe/index.tsx:417`) este caso exato. O quarto sítio é `editar.tsx:407` — **a mesma frase da S-RM22 repetida na tela**, e consertar só a do servidor deixaria a tela contradizendo a decisão pela segunda vez |
...
| ~~S-RM25~~ | 🟡 | `financeiro/projecao.tsx:82` → `:91` → `:94` | ~~**A S-RM18 no quinto arquivo, e ela nasce de uma EXCLUSÃO malfeita — a minha.** O E258 excluiu o `projecao.tsx:57` com razão certa (é o dia CONFERIDO do caixa, estado inicial que não deve mudar sozinho), e a exclusão foi escrita **lendo a linha, não o arquivo**: 25 linhas abaixo, `const hoje = hojeLocal()` alimenta a `janela` que vira `getListPagamentosQueryKey`, e é a classe inteira da S-RM18. O E264 não fechou porque `financeiro/**` era território do E261/E262. **A lição está no relatório dele: achado tem âncora obrigatória, exclusão não tem**~~ | ✅ `d7e8690d` (E266) — **o dia entra como TETO, e isso separa esta das outras oito**: `de`/`desde` são piso e a chave velha só pede janela mais LARGA, mas `ate` que fica para trás ESCONDE — o recebimento de hoje sai do realizado desde a âncora e a curva do caixa nasce de um saldo menor do que o que a loja tem. As âncoras andaram dois dígitos (`:84` → `:93` → `:96`). O `diaConferido` do `:60` fica com `hojeLocal()`, agora com a razão escrita no arquivo |
| ~~S-RM26~~ | 🔵 | `vestidos/utilizacao.tsx:63-66` | ~~**A mesma classe atrás de um helper de uma linha, e ela escapa das DUAS grafias da varredura.** O `useMemo` congela o dia por trás do `diaISO`, e tanto a grafia do E258 (dentro de `useMemo`) quanto a do E264 (o `const` que alimenta uma `…QueryKey`) procuram o NOME `hojeLocal` — uma indireção de um salto as cega. A régua do E264 segue **um** salto e isso está escrito nela; este é o caso que pede o segundo~~ | ✅ `d7e8690d` (E266) — **a sobra dizia que ela escapa das duas grafias, e a medição mostrou o inverso do conserto: agora as DUAS a pegam.** Devolvido o arquivo à forma antiga com a régua nova no lugar, `utilizacao.tsx:77` é acusado pela varredura do `useMemo` E pela da `queryKey` — o mesmo sítio pertence às duas classes, porque o memo do `:75` vira a chave do `:82`. O helper recebe o dia por parâmetro; a régua ganhou `lentesDoDia`, que soma ao `hojeLocal` todo helper do PRÓPRIO arquivo cujo corpo lê o dia. Ela não atravessa módulos de propósito — isso é a S-RM27 |
| ~~S-RM27~~ | 🔵 | `noivas/helpers.ts:41` · `reservas/helpers.ts:61` | ~~A mesma indireção **no display**, em seis telas: o helper chama `hojeLocal()` para contar dias, e quem o chama não sabe que está lendo o dia. Não é chave de consulta — o dano é a contagem exibida envelhecer numa aba aberta pela virada —, e por isso é 🔵 e não a mesma linha da S-RM26~~ | ✅ `d7e8690d` (E266) — seis telas confirmadas, **e a sobra não nomeia o SEGUNDO salto**: `ajustes/index.tsx` e `ajustes/[ajusteId].tsx` chegam ao contador por `lib/ajustes-prazo.ts:86` e `:115`, que é onde a régua do `useMemo` já não alcança. Os dois contadores ganharam um `hoje` opcional com o default de sempre (quem não é tela segue igual), as seis telas passam o dia do `useDiaLocal()` — **duas já o liam** — e a régua nova mede a CHAMADA: um argumento só é o dia implícito |
| ~~S-RM28~~ | 🟡 | `financeiro-core/src/datas.ts:188` × as 4 telas de janela | ~~**A troca de pontas do `resolverIntervalo` sobrevive ao conserto do E261, e ela é silenciosa.** Fechada a fresta do frame, ainda dá para alargar a janela em UMA edição: digitar `De = 2026-08-31` sobre um `Até = 2026-01-01` devolve oito meses, e o campo "De" passa a exibir **outra data** — a que o `resolverIntervalo` decidiu —, sem aviso nenhum, no mesmo botão de mão única. A troca existe para ser tolerante com URL montada à mão; o preço é que ela reinterpreta o gesto da pessoa sem dizer~~ | ✅ `73654f05` (E265) — **o conserto NÃO é do núcleo**, e a sobra pedia que fosse: o `resolverIntervalo` tem seis chamadores e dois são rotas (`financeiro.ts:1215` e `:1291`, tolerância documentada em `lib/intervalo.ts:41`). Entrou o `pontasTrocadas` ao lado dele, pela MESMA resolução de defaults, e um aviso num componente só para as quatro telas. **E a sobra via metade do caminho**: a troca dispara também com uma ponta AUSENTE — `?fim=2024-04-04` sozinho devolve `2024-04-04..2026-08-01`, a janela dos 302 recebimentos do E260, alcançável por um link. Quebrado o predicado para a versão ingênua que a sobra descrevia: **3 failed | 25 passed** |
| ~~S-RM29~~ | 🔵 | `hooks/use-escrita-na-url.ts:53-54` | ~~O acumulador do frame vive no MÓDULO, e isso supõe **um roteador por contexto de JS** — verdade hoje (`App.tsx:356`, um `createBrowserRouter`), e o hook se cura no primeiro render se deixar de ser (se a URL real difere da última pedida, a URL ganha). Fica registrado porque a suposição está no código e não no tipo~~ | ✅ `73654f05` (E265) — a suposição saiu do comentário e virou o quarto caso de `escrita-na-url.test.tsx`: a lista de criações de roteador na produção tem de ser exatamente o `App.tsx:356`. **A régua nasceu errada duas vezes na primeira execução** — cobrava os testes (onde `createMemoryRouter` é legítimo em oito sítios) e cobrava um `createBrowserRouter(` que vive dentro de um docblock, que é a S-RM30 de outro ângulo. Vermelho medido com um segundo roteador de produção |
...
| S-RM38 | 🔵 | `__tests__/backup-download-api.test.ts` · `__tests__/e236-manuais-api.test.ts` (no WORKTREE) | **Os dois reprovaram com `expected 200 "OK", got 410 "Gone"` no worktree do E262, e o agente provou que era anterior ao épico** revertendo `artifacts/` à base. **Medido pelo orquestrador no `main` fora de worktree: os dois PASSAM — 2 arquivos, 11 testes —, e a suíte inteira deu 1905/272 verde com o épico dentro.** Logo **não é defeito do repositório: é ambiente.** O 410 vem do callback de erro do `res.download` (`routes/manuais.ts:45`), e não é arquivo faltando — o `vendedora.pdf` está versionado com 5.639.725 bytes e o `existsSync` que guarda a porta passa. A causa provável, NÃO provada, é carga: o worktree rodava com quatro agentes e duas suítes do orquestrador na mesma máquina. **Fica aberta como fragilidade de régua, não como defeito de porta** — dois vermelhos que viram paisagem apagam a pergunta "quebrei alguma coisa?" (regra 18) | aberta (E262, 17/08) — **reclassificada de 🟠 para 🔵 pela medição no `main`** |
| S-RM39 | 🔵 | `financeiro/folha.tsx:524` | A `pendencia` tem **240 caracteres numa linha só** — três ternários de plural interpolados na mesma string. Não é defeito de comportamento; é uma frase que ninguém lê no diff, e **qualquer conserto nela é feito às cegas** (o vermelho da régua do E262 precisou de uma janela para caber) | aberta (E262, 17/08) |
