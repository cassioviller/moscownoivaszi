...
(ver `rodada-2-consolidado.md` e o fecho em `RODADA-2.md`).

**2026-08-11, mais tarde: resta UMA.** A **S-M10 FECHOU** — não aqui, mas no
**E169** da trilha da ótica dos papéis (`fe8afdd`), que a absorveu por tocar as
mesmas duas pontas. O contrato foi decidido como ela pedia: `LeadInteresseInput`
admite `null` em `algoAMais`, `naoQuerUsar` e `tetoOrcamento`, a tela manda
`null` quando o campo fica vazio, e **o campo AUSENTE continua significando "não
mexi"** — a distinção importa porque o `definirTeto` do `e2e/19` manda só o
teto, e ler "PUT substitui tudo" apagaria dados do seed compartilhado. Limpar o
teto de R$ 8.000,00 e salvar agora apaga o teto; antes o toast dizia sucesso e o
valor velho ficava.

1. ~~**S-M10**~~ — **fechada no E169 (`fe8afdd`)**, trilha da ótica dos papéis.
2. **S-M17** — espera dados de um banco de instalação real. Gente, não
   código. **É a única sobra de código aberta desta trilha.**
...
| ~~S-M10~~ | ~~**Apagar o teto de orçamento é ignorado em silêncio, com toast de sucesso** (`pages/noivas/[leadId]/interesses.tsx:161` × `routes/leads.ts:661-664`). Campo vazio vira `undefined`, some do JSON, e o `set: { ...insertData }` do `onConflictDoUpdate` preserva o valor antigo. Nem um `null` explícito resolveria pela tela: o contrato não o admite — o conserto é dos dois lados~~ **FECHADA no `fe8afdd` (E169** da trilha da ótica dos papéis, que a absorveu por tocar as mesmas duas pontas). O contrato passou a admitir `null` nos três campos (`algoAMais`, `naoQuerUsar`, `tetoOrcamento`) e a tela manda `null` quando o campo fica vazio. **O ausente continua sendo "não mexi"**, e a distinção não é preciosismo: o `definirTeto` do `e2e/19` manda só o teto, e ler o PUT como "substitui tudo" apagaria dados do seed compartilhado (regra 16). Vermelho antes: `expected 400 to be 200` | ✅ | localizador de correção · `fe8afdd` (E169) |
