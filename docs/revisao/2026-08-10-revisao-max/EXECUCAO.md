...
**18 sobras: 8 fechadas.** **As duas 🔴 e as seis 🟠 fecharam — não resta
nenhuma linha acima de 🟡.** As dez abertas são 🟡: sete do diagnóstico
(S-M6, S-M8, S-M9, S-M10, S-M12, S-M13, S-M14) e três nascidas da execução
(S-M16, S-M17, S-M18).
...
| S-M18 | **O relatório afirmou "mais três sítios com a mesma escrita" do S-M7 — e a enumeração se perdeu com a transcrição.** A seção "As três formas que se repetem" diz que o check-then-write fora de transação tem mais três ocorrências além do fechado em `75882f0`, sem dizer onde. Recuperá-los não exige repetir a rodada: é uma varredura da regra 22 — cruzar todo `router.post`/`patch` que lê guarda no pool global (`await db.select` antes de `db.transaction`) e escreve o que a guarda protege dentro dela. O padrão de conserto já existe em dois sítios (S33 e S-M7) para copiar | 🟡 | execução da S-M7 |
