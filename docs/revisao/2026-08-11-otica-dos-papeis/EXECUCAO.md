...
| S-O38 | **O botão "Lookbook" leva a uma página que não existe**, em dois lugares: a barra "Atendendo…" (`components/barra-atendimento.tsx:79`) e a linha do atendimento EM_ATENDIMENTO (`pages/atendimentos/index.tsx:396`). Os dois apontam `/noivas/:leadId/lookbook`, e **não há essa rota** no `App.tsx` — o lookbook é um CARD da ficha (`noivas/[leadId]/index.tsx:606`), não tela. Cai no catch-all e a vendedora lê "Não encontramos esta página" **no meio de um atendimento**. O botão irmão "Interesses" funciona, porque a rota dele existe. Nenhum E2E cobre a URL. Achado ao levantar o caminho da vendedora para o manual | 🟡 | manual da vendedora | aberta |
