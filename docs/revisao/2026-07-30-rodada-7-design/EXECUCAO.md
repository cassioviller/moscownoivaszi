...
| # | O quê | Peso | Origem |
|---|---|---|---|
| ~~S-D1~~ | ~~**O script de captura de telas não existe no repo.** As 81 capturas de hoje foram geradas por um script de scratchpad que se perdeu (o diretório nasceu `undefined/` — a env var do destino não existia). Recriar como `scripts/` versionado, declarando ambiente (browser, locale, viewport) no manifest — é a ferramenta de verificação visual desta rodada e das próximas.~~ **FECHADA em `cbe79f6`** (agente B3 da fase 3; o orquestrador validou COM O APP DE PÉ — fechar a sobra do script perdido com um script nunca executado seria recriá-la): `scripts/capturar-telas.ts`, 78 capturas em ~90 s, env obrigatórias falhando alto com a lição do `undefined/` no texto, locale pt-BR FIXADA (contexto + `--lang`), e o portal da noiva indo para `falharam` sem `NOIVA_TOKEN` em vez de sumir. Como rodar: `scripts/README.md` e `replit.md`. | 🟡 | montagem da rodada |
| ~~S-D2~~ | ~~**O manifest da captura não declara ambiente.** Viewport foi recuperado dos PNGs (1280×800 / 390×844); navegador segue desconhecido. A trilha E provou a locale pelos próprios PNGs: interface **en-US** ("July 2026" em `financeiro-folha--390.png`, `mm/dd/yyyy` em `financeiro-auditoria--390.png`). O script recriado (S-D1) deve declarar isso no manifest.~~ **FECHADA em `cbe79f6`, nas duas metades:** o script novo grava o bloco `ambiente` completo no manifest de saída (navegador+versão, locale, timezone, viewport, tema, data), e o manifest EXISTENTE das 81 ganhou o mesmo bloco com a verdade parcial — viewport dos PNGs, locale en-US provada por eles, navegador marcado `desconhecido`. Verdade parcial escrita vale mais que campo ausente, e nada foi recapturado. | 🔵 | montagem da rodada |
...
| ~~S-D24~~ | ~~**O spec 19 muta estado do seed e não restaura:** `e2e/19-orcamento-teto.spec.ts:62` deixa o teto de orçamento do lead do seed em 100.000 — não é lixo crescente, mas é estado que outro spec pode herdar.~~ **FECHADA em `f72628c`.** O `beforeAll` captura o que havia (valor E existência da linha — o interesse do `e2e-lead-1` só existe porque o próprio spec o criou via upsert, em 21/07) e o `afterAll` restaura os dois casos. O resíduo saiu uma vez (100000.00 → NULL, anotado — regra 16), e a prova é a medição depois do run completo: o spec escreveu 100 e 100000 durante os testes e o banco terminou em NULL. | 🔵 | execução E146 |
| ~~S-D25~~ | ~~**As cabines de runs antigos do spec 18 são desativadas, nunca apagadas**~~ (`e2e/18-agenda-grade.spec.ts:39-45`) — lixo legado `Cabine E2E {timestamp}` que nenhum spec reclama. O afterAll do E146 impede as novas; as velhas pedem uma limpeza única. **Confirmada em 2026-08-05: as 186 cabines continuam lá.** A faxina da S-A13 (`e2bb58b`) ficou de propósito em vestidos e atributos — cabine desativada não é peça sem referência, e a guarda dela não serve; esta limpeza precisa da própria, que saiba distinguir `Cabine E2E {timestamp}` de cabine de loja viva. **REMEDIDA em 2026-08-06, e o alvo que ela nomeia é 5 linhas — o passivo de verdade é OUTRO e são 220.** Contagem por família em `psql`, sobre 240 cabines: `Cabine E2E {timestamp}` (o spec 18, que a sobra descreve) são **5**, quatro delas inativas; **220 são `e<NN>-<timestamp>`, todas ATIVAS**, e as 230 da loja do seed são o que a agenda daquela loja desenha. Elas não vêm do spec 18: vêm de **cinco specs que criam cabine por execução e não a apagam** — `e22` 68, `e25` 66, `e24` 36, `e57` 26, `e59` 24, desde 30/07. **E o conserto já tem prova de que funciona:** o `e24` parou em 30/07, no dia em que ganhou `delete(cabinesTable)` no `afterAll` (`24-dias-funcionamento.spec.ts:42`) — os outros quatro que crescem (`22`, `25`, `57`, `59`) não têm a linha, e os três que têm (`24`, `49`, `55`) escrevem a mesma limpeza em **três grafias diferentes**, que é a regra 26 pedindo uma régua. O épico é: a linha nos quatro, a faxina única das 225, e uma varredura cobrando que spec que cria cabine apague a sua. **FECHADA em `80d7d35`, nas quatro peças que a remedição pediu:** a régua única `apagarCabineCriada` em `e2e/helpers.ts` (os três specs que limpavam escreviam três grafias — regra 26 — e convergiram; os quatro que não limpavam ganharam a chamada), a faxina única com guarda de cabine VAZIA (`atendimentos.cabine_id` é CASCADE, e apagar cabine com história levaria a história junto — DELETE 224, restam 16, dump antes), e a varredura `varredura-cabines-do-e2e.test.ts` com piso de população (61 specs, 8 criando). **Provada por uma suíte completa depois da faxina: zero cabines de fixture no fim.** A primeira versão da régua custou dois runs — `import()` dinâmico atravessa a transpilação do Playwright e derrubou 7 specs com `exports is not defined`, e o crash no meio do `afterAll` do 55 deixou o contrato de R$ 8.400 que derrubou o spec 37 no run seguinte (`Expected 1550, Received 4340`): a demonstração involuntária da classe que o épico fecha. | 🔵 | execução E146 · remedida em 2026-08-06 |
...
| ~~S-D39~~ | ~~**`bloqueioId` é gravado no `.state.json` e nenhum spec o lê.**~~ `global-setup.ts:280` grava `bloqueioId: "e2e-bloqueio-1"`, a interface `E2EState` (`helpers.ts:6-19`) **não declara o campo**, e os 59 specs não o usam — `23-prova-data-real` e `48-avaria-vira-parcela` criam bloqueios próprios. A fixture existe e serve ao `13-onda2-telas.spec.ts:52-60`, que a acha pelo NOME da noiva, não pelo id. Ou o campo entra na interface, ou sai do state. **FECHADA em `13d1204`: saiu do state.** O 13-onda2 acha o bloqueio da fixture pelo nome da noiva e os specs 23/48 criam os seus — superfície que ninguém consome é promessa que alguém um dia acredita. A interface ganhou o comentário dizendo por quê. | 🔵 | onda 2 de 2026-08-06 (S-D27) |
| ~~S-D40~~ | ~~**O número da S-A12 envelheceu de novo, e o crescimento tem taxa.**~~ A conferência de 05/08 mediu 180 cabines na loja do seed; hoje são **206** (de 216 no banco inteiro), e só **3** têm id derivado do seed (`84e539bd-…-cabine-{1,2,3}`) — as outras 203 são acúmulo de spec. As lojas de fixture passaram de 20 para **23**, em 25. A faxina da S18/S-A13 não toca `lojas` nem `cabines`: o crescimento é de **~26 cabines por semana**. Casa com a S-D25, que mede a mesma população por outro recorte. **REMEDIDA horas depois, e o crescimento tem CAUSA nomeada, não só taxa:** 240 no banco inteiro, **230 na loja do seed**, e **220 são `e<NN>-<timestamp>` de cinco specs do E2E** que criam cabine por execução e não a apagam (`e22` 68, `e25` 66, `e24` 36, `e57` 26, `e59` 24, desde 30/07). A taxa não é por semana, é **por RUN**: cada suíte completa deixa quatro cabines para trás — as dos specs 22, 25, 57 e 59. O `e24` provou o conserto: parou de crescer em 30/07, quando ganhou o `delete` no `afterAll`. O trabalho está descrito na S-D25, que é onde ele fecha. **FECHADA em `80d7d35`, junto com a S-D25** — mesma população, mesmo conserto. Depois da faxina e de uma suíte completa: 16 cabines no banco, zero de fixture, e a taxa por run caiu a zero porque os oito specs que criam cabine agora apagam a sua. | 🔵 | onda 2 de 2026-08-06 (S-D27) · remedida em 2026-08-06 |
...
| ~~S-D30~~ | ~~**Catorze varreduras ainda enumeram pelo DISCO com `readdirSync`, e a pior é a que o METODO cita como caso exemplar da regra 22.** A onda 1 consertou três (`s28-assert-tautologico-unit`, `varredura-reguas`, `erros-regua-varredura`, todas passando a `arquivos-versionados.ts` com `git ls-files -z`); ficaram **seis no servidor e oito no frontend**. A prova de que a fresta é real: com `artifacts/moscow-noivas/src/tmp/copia.ts` no disco e fora do git (`.gitignore:5`), as sondas de HEAD reprovavam por um arquivo que não é do repositório. A `s36-gate-da-tela-unit.test.ts:36` e `:73` é a mais cara — é a varredura que o METODO apresenta como exemplar ("achou dois defeitos na primeira execução, um deles vivo num perfil PADRÃO: a Recepção via Criar reserva e levava 403") e enumera telas E rotas pelo disco, nas duas pontas. **Os oito do frontend não são escopo perdido, são decisão de dono:** consertá-los pede uma SEGUNDA cópia de `arquivos-versionados.ts` dentro de `artifacts/moscow-noivas`, porque não há pacote de utilitário de teste compartilhado — e "cópia correta continua sendo cópia" é a frase que a própria `varredura-reguas.test.ts:141` usa. Mesma família da S29.~~ **FECHADA em `c98341e`** (agente B1): eram **16 arquivos, 19 call-sites** (6 servidor + 10 frontend, não 8), todos migrados — o frontend por cópia de `arquivos-versionados.ts` com a decisão no cabeçalho (sem pacote de util de teste; o terceiro consumidor promove). Disco e git provados idênticos nos 11 escopos antes de migrar. | 🟡 | onda 1 de 2026-08-06 (S-D7) |
| ~~S-D31~~ | ~~**Nenhuma varredura deste repositório tinha piso de população, e varredura sobre conjunto vazio aprova tudo em silêncio.** Se a enumeração devolver zero por qualquer motivo — pasta renomeada, âncora de `import.meta.dirname` apontando para onde um arquivo não está mais, um `git ls-files` que falhe — o `expect(ofensores).toEqual([])` fica VERDE por não ter olhado nada. É o modo de falha mais caro de uma sonda, e é a mesma família da S30: a sonda diz guardar um número e guarda outra coisa. A onda 1 pôs piso nas três que tocou (`> 200` arquivos de teste na da S28, `> 200` fontes na `varredura-reguas`, `> 100` sites de 404 na `erros-regua`). As outras catorze não têm.~~ **FECHADA em `c98341e`** (agente B1): piso em TODAS as 16, cada um com o número medido em 07/08 e folga para baixo (rotas 20, endpoints 194, telas 66, fontes 166, snapshots 11, migracoes 36, ...), e o motivo no comentário de cada uma. | 🟡 | onda 1 de 2026-08-06 (S-D7) |
| ~~S-D32~~ | ~~**Quatro números diferentes circulam como "os formatadores `Intl` do sistema", e nenhum diz seu recorte.** A S30 dizia **quinze**; a conferência de 2026-08-05 corrigiu para **17**; o registro da sessão de 2026-08-06 mediu **25** depois da faxina de worktree; e a conta que a sonda faz é **36** em código de aplicação (17 no passivo + 19 nos sete arquivos-régua, com `formatos.ts` sozinho em 10), ou **46** se a varredura contar testes e E2E. Não são erros: são recortes diferentes que ninguém nomeou. Enquanto o recorte não estiver escrito ao lado do número, qualquer um deles vira "o número de formatadores" na próxima leitura — e foi exatamente assim que a S30 nasceu dizendo quinze.~~ **FECHADA em `c98341e`** (agente B1): os recortes estão NOMEADOS na seção 4 da varredura-reguas, medidos em 07/08 — passivo herdado **17**, arquivos-régua **19** (formatos.ts com 10), aplicação **36**, com testes e E2E **45** (não 46). A população de fontes foi remedida no mesmo gesto: 237 → 235, com as entradas e saídas verificadas por `git log --diff-filter`. | 🔵 | onda 1 de 2026-08-06 (S-D7) |
| ~~S-D33~~ | ~~**A `varredura-reguas.test.ts` pula do item 2 para o item 4 nos comentários de seção, e não há item 3.** O arquivo tem `// ─── 1. "hoje" no relógio de quem executa ───` (`:77`), `// ─── 2. dinheiro lido fora da régua pt-BR ───` (`:107`) e, em seguida, `// ─── 4. formatador declarado fora dos arquivos-régua ───` (`:142`). Ou uma das cinco assinaturas do E111 foi removida sem o comentário, ou a numeração nunca casou com o consolidado — e a numeração existe justamente para casar a sonda com o diagnóstico. **Ninguém mexeu de propósito:** renumerar sem saber qual era o 3 apaga a pergunta em vez de respondê-la.~~ **FECHADA em `c98341e`** (agente B1), com a resposta vinda do histórico: **o item 3 nunca esteve no arquivo** — `git show fc2182a` mostra o blob nascendo com 1, 2 e 4, e o adendo do E111 (`E111.md:354-366`) lista a terceira assinatura, `router.use(fn)` sem path, como a que virou sonda de COMPORTAMENTO: a `varredura-fronteira-loja-api`, criada no MESMO commit (grep não prova guard; o 403 por recurso prova). O buraco ganhou a seção "3." dizendo isso, sem renumerar. | 🔵 | onda 1 de 2026-08-06 (S-D7) |
| ~~S-D34~~ | ~~**O `Input` fica em 36px no mobile — a MESMA classe da S-D18, no primitivo que ninguém mediu.** `input.tsx:11` traz `h-9` cru, o defeito que o `SelectTrigger` acabou de perder em `3c463bb`. A medição do E137 não o pegou porque contava alvos por PAPEL clicável, e campo de texto também é alvo de dedo. O `textarea.tsx:12` já está certo (`min-h-[60px]`). O conserto é uma palavra, a mesma grafia `min-h-11 md:min-h-9` de `button.tsx:39`. Ficou fora da onda 1 por escopo de arquivo, não por mérito.~~ **FECHADA em `f4cb527`** (agente B2): a grafia de `button.tsx` aplicada ao primitivo — 44px no mobile, no-op no desktop —, com o alcance medido (41 arquivos importam o `Input`, 36 em `pages/`) e a varredura de alvo de toque ganhando o caso ANTES do conserto, com o vermelho literal registrado. **O quarto primitivo da família virou a S-D47** (`toggle.tsx`). | 🟡 | onda 1 de 2026-08-06 (S-D18) |
| ~~S-D35~~ | ~~**O `AlertaCaixa` é o TERCEIRO cartão que aparece de repente no topo do painel, e fica ACIMA dos dois que a S-D10 nomeia.** Ele faz `if (!data) return null` sobre a própria consulta (`alerta-caixa.tsx:31`) e é renderizado antes dos avisos (`dashboard.tsx:322`). O silêncio enquanto carrega é decisão explícita do E103 ("nada a dizer é nada na tela") e não se discute — mas o LUGAR dele não é reservado: quando o alerta existe, ele empurra a página inteira para baixo depois da pintura, que é exatamente o que os outros dois faziam até `3c463bb`.~~ **FECHADA em `f4cb527`** (agente B2), com a MESMA forma dos dois irmãos: esqueleto com a frase fixa invisível reservando o lugar, decidido por `estadoDasConsultas` — e a decisão do E103 intacta: erro segue calado, o que mudou é o layout não saltar. Vale nos dois sítios (dashboard e hub de fluxo). | 🟡 | onda 1 de 2026-08-06 (S-D10) |
| ~~S-D36~~ | ~~**A tela de perfis globais edita e não cria:** `POST /admin/perfis` existe no servidor (`admin.ts:230`) e o app tem ZERO usos de `useCreatePerfil` (`admin/perfis.tsx:31`). É o que torna o vazio da S-D9 um beco — a tela do superadmin lista, deixa editar cada linha e não tem por onde repor o que falta. Decisão de produto, não defeito de código, mas é a razão pela qual o vazio novo ficou sem botão. **DECIDIDO em 2026-08-07** (folha, respondida na sessão): **a tela ganha o botão** — ligar a tela do superadmin ao `POST /admin/perfis` que já existe. Deixou de ser pergunta; é sobra de código com a decisão escrita.~~ **FECHADA em `74c540f`.** O "Novo perfil" fica no cabeçalho, fora do ramo da lista — no vazio da S-D9 é onde ele mais faz falta —, e o perfil nasce com TUDO desligado: quem liga é o superadmin, na matriz que já existia. O E2E do spec 46 prega o gesto inteiro e o servidor confirmando o nascimento sem acesso; o spec apaga o que criou. | ✅ | onda 1 de 2026-08-06 (S-D9) · decisão 2026-08-07 · `74c540f` |
| ~~S-D37~~ | ~~**A listagem de parcelas abertas embute o `Lead` INTEIRO em cada parcela, e a fila de cobrança usa três campos dele** (nome, whatsapp, leadId): `financeiro.ts:141` monta `with: { contrato: { with: { lead: true } } }`, `cobranca.ts:63` consome três. É por aqui que a **S-D13** fecha barato — basta a rota chamar o agregado `ultimoContatoPorLead` que `leads.ts:59-70` já tem — e é também tráfego que ninguém mediu: cada parcela aberta carrega um objeto de noiva completo.~~ **FECHADA em `a0e8cd4`, junto com a S-D13:** `ParcelaContrato.lead` virou `ParcelaLead` `{noivaNome, whatsapp, ultimoContatoEm}` no contrato, o teste de API afirma as três chaves por `Object.keys`, e o typecheck não acusou consumidor nenhum fora delas. **O emagrecimento teve efeito colateral que virou prova:** o dashboard passou a montar mais rápido e o spec 13 caiu com strict mode violation — `getByRole` casa nome por SUBSTRING e "Atendimentos" vive em três links; o assert só passava enquanto os cartões não tinham montado. Locator escopado à sidebar. | 🔵 | onda 1 de 2026-08-06 (S-D13) |
| ~~S-D41~~ | ~~**O seed imprime "seg–sáb, 9h–19h" e grava domingo–sábado, 9h–20h.** `scripts/seed.ts:54` traz o literal `"seg–sáb, 9h–19h"` na linha "Horário de funcionamento" do resumo; o `HORARIO_PADRAO` que ele acabou de aplicar (`configuracao-inicial.ts:148-157`) diz `diasFuncionamento: [0,1,2,3,4,5,6]` e `atendimentoFechamentoHora: 20`. Conferido no banco descartável `sd38_virgem`: o seed imprimiu a frase e a linha gravada tem `[0,1,2,3,4,5,6]` e `20`. **Os dois números que a linha erra são exatamente os dois que a S-A8 mudou** depois de a dona responder — domingo com hora marcada e o expediente até as 20h —, e é essa linha que ela lê para conferir se o ateliê ficou configurado do jeito dela. O resumo é a única prova que o script dá; ele diz que domingo está fechado quando o sistema vai abrir.~~ **FECHADA em `60adc7c`, e quem a pegou foi a régua nova do banco virgem — nenhuma suíte podia.** O resumo do seed só se lê numa instalação NOVA, e a frase estava cravada no script. Vermelho literal do `scripts/banco-virgem.ts`: *"o banco guarda [0,1,2,3,4,5,6] e a linha diz seg–sáb, 9h–19h"* e *"o banco guarda fechamento 20h e a linha diz 9h–19h"*. Agora a linha sai de `descreverHorario` sobre o que foi gravado — **"todos os dias, 9h–20h"** —, com teste de unidade casando a descrição com o `HORARIO_PADRAO` e um segundo que reprova se o default voltar a ser o antigo. | 🟡 | onda 2 de 2026-08-06 (S-D38) |
| ~~S-D42~~ | ~~**A hora de fechamento do E2E é a que o banco tiver, e os dois bancos discordam.**~~ A fixture de `global-setup.ts` grava cinco campos da regra e **não** grava `atendimentoFechamentoHora`: o banco de dev está com **19** (medido em `psql`, linha `e2e-regra-disp`) porque nasceu antes da S-A8, e um banco novo nasce com **20**, que é o default de hoje. O `18-agenda-grade.spec.ts:102` comenta "expediente padrão 9h–19h" e afirma só a presença de `09:00` e `18:30`, que existem nas duas configurações — então a divergência passa em verde nos dois bancos, e nada no repositório diz qual é o expediente que a suíte pretende testar. Ou a fixture fixa a hora, ou o spec para de afirmar um expediente que ele não escolheu. **FECHADA em `13d1204`, pelas DUAS pontas:** o `AJUSTE_E2E` grava `atendimentoFechamentoHora: 20` — a decisão da dona na S-A8, não uma preferência de teste — e o spec 18 passou a pregar o expediente escolhido (o 19:30 é o último slot do fechamento às 20, e o 20:00 não existe). Medido depois do run: o banco de dev guarda 20, e a divergência entre os dois bancos morreu. A régua do banco virgem rodou por causa do `global-setup` e passou inteira. | 🔵 | onda 2 de 2026-08-06 (S-D38) |
| ~~S-D44~~ | ~~**`lib/api-spec/orval.config.ts` é o último TypeScript do repositório fora de todo typecheck.** O `tsconfig.json` da raiz tem `files: []` e referencia seis pacotes de `lib/` — `api-spec` não é um deles —, e o pacote não tem script `typecheck`, então o `--filter "./scripts"`/`--filter "./artifacts/**"` da raiz nunca o alcança. É **1 arquivo**, e é o que configura o codegen: se ele quebrar, quem paga é o `orval`, que já apagou `generated/` ao falhar uma vez (E142, regra 15). Mesma fresta da S-D23 (63 specs de `e2e/` sem typecheck) e da que o `60adc7c` fechou em `scripts/` — e a lição comum é a mais cara: **`--if-present` acerta o pacote e não faz nada, em silêncio.** A régua que falta não é o tsconfig do `api-spec`: é uma varredura que case a lista de pacotes com quem tem cobertura, para o quarto caso não nascer.~~ **FECHADA em `c092e71`, nas duas metades.** O pacote: tsconfig + script `typecheck` + filtro na raiz — e no primeiro giro o tsc achou defeito real: `prettier: true` nos dois blocos é opção que o orval 8 ignora em silêncio (virou a S-D48, com o custo de ligar medido: 232 arquivos). A régua: `sd44-typecheck-cobre-pacotes-unit.test.ts` casa os 11 pacotes com quem o typecheck alcança, reprova pacote descoberto E filtro que acerta pacote sem script (o silêncio do `--if-present`), e trava a lista de exclusões por nome. Vermelho antes literal: `lib/api-spec (@workspace/api-spec)`. | ✅ | onda 2 de 2026-08-06 (S-D43) · `c092e71` |
| ~~S-D45~~ | ~~**O spec 41 CANCELA os contratos que cria em vez de apagá-los, e a família S-D25 tem uma tabela a mais.** `41-colocacao-comissao.spec.ts:64-70` marca `status: "CANCELADO"` no `afterAll` — correto para tirar as vendas do ranking e das pendências (E53), mas as linhas ficam: **274 contratos `E2E Colocacao` (todos CANCELADO) e os 274 leads deles, acumulados de 21/07 a 06/08** — eram 272 antes do run que fechou o épico da S-D25 e 274 depois: a taxa de +2 por passada foi medida na própria sessão. Zero parcelas em todos (medido). Inerte para as telas de hoje; é população que toda consulta de contratos carrega e todo dump arrasta. O conserto é a mesma forma da S-D25: apagar contratos → leads no `afterAll`, e a faxina única das 274+274 com guarda de "CANCELADO, sem parcela, nome do spec".~~ **FECHADA em `a38f994`.** O afterAll passou a apagar contratos → leads (RESTRICT manda a ordem) e a passada de verificação terminou com resíduo ZERO. A faxina única levou **300+300** — eram 274 na medição da sobra, +26 em um dia: a taxa de +2 por passada paga treze vezes — com a guarda tripla (assinatura no nome, CANCELADO, nenhuma parcela) e zero casos de borda nos dois sentidos. | ✅ | épico da S-D25 (`80d7d35`) · `a38f994` |
| ~~S-D46~~ | ~~**26 lojas de fixture zumbis seguram o ecossistema que as faxinas não alcançam.** `Loja Teste %`/`Loja Vazia %` de suíte interrompida — o `afterAll` do vitest não roda quando o run morre, e `limparFixture` limpa tudo na ordem certa quando roda. A S18 limpou usuários órfãos, a S-A13 o acervo, a S-D25 as cabines: **nenhuma tocou `lojas`**, e a loja viva segura o resto por FK — **44 dos 48 perfis do banco eram `Perfil (Admin) Teste` pendurados nelas** (medido no épico da S-D26), e a S-D27 provou que uma delas herdaria a suíte inteira se a loja do seed sumisse. A faxina precisa da guarda inversa da do `DELETE /admin/lojas`: apagar SÓ o que casa a assinatura de fixture E não tem contrato/parcela com dinheiro real — os 33 CASCADE (S33) levam o resto.~~ **FECHADA em `fda7cf2`.** A guarda inversa que a sobra pediu: assinatura de fixture E zero dinheiro (0 parcelas PARCIAL/PAGA, 0 pagamentos nas 26 — o único contrato ATIVO tinha R$ 10.000,00 e nenhum centavo movido). Saíram **26 lojas + 44 perfis + 24 usuários** `@teste.local`, os perfis com tripla checagem de referência e os usuários com o DELETE da S18 verbatim. Ficaram 2 lojas, 4 perfis, 26 usuários, zero órfãos — e o E2E completo rodou verde depois: 161/161. A recorrência é só de run interrompido: `limparFixture` já limpa certo quando roda, e a eleição da suíte é por id (`e01bff4`). | ✅ | épico da S-D26 (`5be1895`) · `fda7cf2` |
| ~~S-D47~~ | ~~**`toggle.tsx:17` é o QUARTO primitivo com `h-9` cru, e está em uso real.** A família S-D18 (SelectTrigger, `3c463bb`) → S-D34 (Input, `f4cb527`) tem mais um caso: o `ToggleGroupItem` que `noivas/index.tsx:24` usa é interativo e fica em 36px no mobile. E `breadcrumb.tsx:98` tem `h-9 w-9` (via `cabecalho-detalhe.tsx`) — a conferir se o elemento é clicável antes de mexer. O conserto é a grafia de `button.tsx` de novo; a varredura de alvo de toque já sabe cobrar primitivos (o caso do Input mostra a forma).~~ **FECHADA em `015a0db`.** As três variantes de tamanho ganharam a grafia do E137, na altura E na largura (`min-w-11 md:min-w-9` — toggle pode ser só ícone); o toggle-group herda `toggleVariants`, então uma grafia cobre os dois. A conferência que a sobra pedia sobre o `breadcrumb.tsx:98` matou a segunda metade: `BreadcrumbEllipsis` é `<span role="presentation" aria-hidden>` com **zero consumidores** — não é alvo. A varredura alvo-de-toque ganhou o quinto caso, ancorado no bloco `size:` (o primeiro achador pegava o `default:` de variant, que é cor). | ✅ | B2 da fase 3 (`f4cb527`) vp · `015a0db` |
| ~~S-D48~~ | ~~**O codegen não formata, e a opção que dizia formatar estava morta.** Achado pelo typecheck que a S-D44 ligou: `orval.config.ts` trazia `prettier: true` nos dois blocos, opção que o orval 8 não conhece — o runtime lê só `formatter` (`config-CXPetEiJ.mjs`: `outputOptions.formatter ?? globalOptions.formatter`) e a ignorava em silêncio. Os `generated/` commitados são o produto SEM formatação: o codegen de 2026-08-07 os **reproduz byte a byte** com a opção removida. Ligar `formatter: "prettier"` foi medido: **232 arquivos, +27.888/−21.341** — churn em arquivos que as varreduras do repositório leem por forma (regra 13), então é decisão, não conserto. Se ligar: no MESMO commit, rodar as varreduras que leem `generated/` (a S36 parseia `get*Url` de `api.ts`) e conferir que os pisos de população não desabam. Se não ligar: nada a fazer — a config já diz a verdade.~~ **FECHADA por DECISÃO em 2026-08-07** (folha de perguntas, respondida na sessão): **o formatter fica DESLIGADO** — a config já diz a verdade desde `c092e71`, e o churn de 232 arquivos não se paga sem motivo novo. | ✅ | S-D44 (`vp`) · decisão 2026-08-07 |
| ~~S-D43~~ | ~~**Nenhuma suíte deste repositório exercita o caminho do banco VIRGEM, e é o único caminho que um ateliê novo percorre.** As três suítes rodam contra o banco de `DATABASE_URL`, que existe desde antes do E147; o `global-setup.ts:47-56` tem um ramo inteiro — "banco sem admin, roda o seed oficial" — que nenhum run executa. Foi ali que a S-D38 viveu, e ela só apareceu porque alguém montou o experimento à mão: `createdb` + `pnpm --filter @workspace/db run push` + o setup, três minutos. A régua que falta é essa sequência num script versionado, contra um banco descartável, afirmando que o setup termina sem erro — sem ela, todo defeito de primeira execução continua invisível até um cliente novo o encontrar.~~ **FECHADA em `60adc7c`: `scripts/banco-virgem.ts`, ~40 s.** Faz a sequência de um ateliê novo — `createdb`, `push`, seed, **confere o resumo impresso contra o que o banco guarda**, sobe o `global-setup` inteiro, roda o seed de novo para provar a idempotência, apaga o banco inclusive se um passo estourar — e guarda/devolve o `e2e/.state.json`, então pode rodar no meio de outra coisa. **Ela nasceu achando:** as três afirmações que reprovaram na primeira execução eram a S-D41 e a S-A12, que nenhuma suíte podia ver. O `replit.md` diz quando rodá-la: antes de publicar, e depois de mexer no seed, no schema ou no `global-setup`. | 🟡 | onda 2 de 2026-08-06 (S-D38) |
