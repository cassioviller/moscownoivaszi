...
| ~~S3~~ | **FECHADA em `86589a0`.** `audit_log.loja_id` virou anulável, e **nulo é o ato global**. No caso da loja, o nulo não é frouxidão: com o id dela, o CASCADE apagaria o registro no mesmo `DELETE` que ele descreve — o rastro morreria no instante em que passasse a importar. O DELETE e a trilha vão na mesma transação, o nome vai desnormalizado no detalhe (depois não há linha para consultar), e há porta de leitura, `GET /admin/auditoria-global` — trilha gravada que ninguém lê é meio conserto. De brinde, o `DELETE /admin/usuarios` ganhou o 404 que não tinha. **A S1 citada aqui já estava fechada pelo E106.** O diagnóstico original: **Ato global de superadmin não deixa trilha.** `registrarAuditoria` exige `lojaId` (`audit_log.loja_id` é `notNull`) e `DELETE /admin/usuarios/:id` é global. Hoje sobra um `req.log.warn`. Registrar em cada loja da pessoa multiplica a mesma ação em N linhas; não registrar exige mudar o schema da trilha. Vale para S1 também. | 🟠 | [E91](execucao/E91.md) "ficou de fora" |
...
| ~~S7~~ | **JÁ ESTAVA FECHADA, e a tabela não sabia — consertada no E115.** O spec traz o comentário nomeando esta sobra (`e2e/25-confirmar-presenca.spec.ts:25-31`): cabine própria por execução (`e25-${Date.now()}`), horário livre por `criarAtendimentoLivre`, e `afterAll` apagando o atendimento. Conferido também por medição: **quatro passadas completas do E2E em 2026-08-05, 156/156 nas quatro.** O diagnóstico original: **`e2e/25-confirmar-presenca` colide consigo mesma entre execuções.** Ela cria o atendimento sempre na cabine fixa `e2e-cabine-1`, às `14:mm:ss` de HOJE, num banco que persiste — quanto mais vezes a suíte roda no mesmo dia, maior a chance de 409 `Registro duplicado ou conflito de dados`. Mesma classe que a sobra da rodada 5 resolveu noutro spec ("recurso próprio por execução"). Um vermelho desses se lê como regressão de dinheiro e não é. | 🟠 (infra de teste) | [E95](execucao/E95.md) |
| ~~S8~~ | ~~**`contratos.ts` mantém `cent`/`reais` locais**, idênticos aos do `financeiro-core`, com dezenas de call-sites — a mesma classe do `parseValor` quadruplicado que o E95 fechou, em volume maior.~~ **FECHADA em `cc9720f`** (agente B4): as implementações eram literalmente idênticas (conferido antes de trocar, inclusive negativos), **24 call-sites** (13 `cent` + 11 `reais`) passaram ao import do core, e as locais saíram. Zero mudança de comportamento por construção. | 🟡 | [E95](execucao/E95.md) vp |
| ~~S9~~ | ~~**O teto de orçamento (E33) compara em reais.** `acimaDoTeto` faz `totais.liquido > teto` em float enquanto o excedente já saiu para centavos. Um centavo no limiar, sem consequência de dinheiro — mas é régua pela metade.~~ **FECHADA em `cc9720f`** (agente B4), com o limiar medido: líquido R$ 950,47 contra teto gravado `950.466` — o float dizia "acima" e o excedente saía **R$ 0,00**, aviso ligado apontando excedente zero. Em centavos, 95047 > 95047 é falso. | 🔵 | [E95](execucao/E95.md) vp |
| ~~S10~~ | ~~**A tela de contrato gera o carnê às cegas.** O `gerar-plano` de lá não tem prévia, e desde o E95 existe a função para mostrá-la — é o F16 aplicado à tela irmã.~~ **FECHADA em `8b9c574`**: a prévia saiu do inline da irmã e virou componente compartilhado (`previa-do-carne.tsx`), a validação virou `planoDaDigitacao` com 4 testes (era lógica pura SEM teste enquanto morava num useMemo), e as duas telas chamam a mesma coisa. A prévia bate com o que o servidor grava por construção — o gerar-plano usa a mesma função do core. | 🟡 | [E95](execucao/E95.md) vp |
| ~~S11~~ | **FECHADA em `523a5bc`** pelo caminho barato que a própria sobra escreveu: teste de paridade. **A contagem dela envelheceu** — dizia "UM enum de cada lado" e hoje são **quatro** (origem da noiva, tipo do atributo, tipo do compromisso, tipo do item do orçamento). Nenhum divergia, então o custo até agora foi zero. Vermelho antes com o caso histórico: tirei `ESTOQUE` do enum do orçamento (o valor que o E154 acrescentou ao contrato) e o teste apontou a diferença literal. O veredito sobre DERIVAR os resolvers continua valendo e não foi revisitado. O diagnóstico original: **D5 não se faz como está escrito — veredito medido, item aberto.** Derivar os resolvers dos 12 formulários do `api-zod` esbarra em duas coisas: o schema gerado descreve o PAYLOAD e o formulário valida a SUPERFÍCIE DE ENTRADA (`entrada`/`numParcelas` não existem no corpo da API), e importar o barril de 261 KB / 539 schemas num bundle que já tem 1,1 MB sem code splitting troca dívida de duplicação por dívida de peso. A duplicação real medida é **um** enum de cada lado, não doze. Caminho barato: teste de paridade dos dois enums, ou reavaliar depois do code splitting do E104. | 🟠 | [E96](execucao/E96.md) |
| ~~S15~~ | **FECHADA em `32d08ba`.** O `include` passou a ser `src/**/*.test.{ts,tsx}`, com DOM por arquivo, testing-library, e o teste do `<Erro>` que a sobra conta ter morrido. **Duas medições corrigiram o plano:** o `jsdom` GLOBAL derrubou `aparencia.test.ts` (que lê o CSS do disco), então o ambiente é por arquivo; e os dois primeiros asserts que escrevi estavam errados sobre código certo — `Error` cru não cai no fallback, e o corpo do erro é `err.data`, não `err.response.data`. **A ressalva:** o que o E111 consertou foi uma DIVERGÊNCIA entre a tela e o servidor (a tela pedia `financeiro.editar` para o que o servidor guarda por `leads`), e teste de componente não pega isso — ele prega o que a tela faz, não se a tela pede o módulo certo. Isso vira sobra própria. O diagnóstico original: **O `vitest` do frontend só coleta testes dentro de `src/lib`.** Teste de componente não chega a ser executado — descoberto ao escrever o do `<Erro>` e ver "No test files found". Ampliar o `include` é infraestrutura de teste e mora no E104, que já vai ligar o typecheck dos testes do front. **O E111 pagou o preço dela pela primeira vez num conserto de PERMISSÃO:** os dois gates das telas do financeiro (`cobranca.tsx`, `receber.tsx` pediam `financeiro.editar` para botões que o servidor guarda por `leads`) ficaram sem teste, e gate de permissão sem teste é a classe de defeito que volta calada. Sobe de 🟡 para 🟠. | 🟠 | [E99](execucao/E99.md) · [E111](execucao/E111.md) |
| ~~S13~~ | ~~**`useBlocker` do react-router não existe neste app.** Ele monta as rotas com `<BrowserRouter>` (`App.tsx:160`), e o `useBlocker` só funciona em data router (`createBrowserRouter` + `RouterProvider`) — fora dele, lança. Sem ele, o D14 protege só o fechar/recarregar a aba: clicar na sidebar com um formulário sujo continua descartando em silêncio. Migrar o roteador toca todas as rotas do app. **CONFERIDA, e o TAMANHO está errado por uma ordem de grandeza — é o que muda a ordem dos épicos.** O `react-router@7.18.1` exporta `createRoutesFromElements`, que aceita a árvore JSX como está: os **59 `<Route>` não mudam nenhum**. Não há `loader` nem `action` (grep = 0), e os três providers já estão FORA do roteador, que é onde o `RouterProvider` os quer. É meia dúzia de linhas em `App.tsx:176-278`, um arquivo. O único cuidado é o `<Suspense>` de `:181`, que sobe. E o hook já está em **8 telas** (`use-confirmar-saida`), não nas 3 do E97: são 8 formulários descartando em silêncio quem clica na sidebar. [conferência de 2026-08-05](../2026-08-05-conferencia-de-sobras.md)~~ **FECHADA em `acdd9b3`, e a sobra errava o TAMANHO — para mais.** "Toca todas as rotas do app" sobre **meia dúzia de linhas em um arquivo**: `App.tsx` virou `createBrowserRouter(createRoutesFromElements(…))` sem mexer em nenhum dos **59** `<Route>` (`git diff -w` dá 98+/16-, e as 88 linhas restantes do diff completo são reindentação), porque não há loader nem action e os providers já estavam fora. E o hook já estava em **8 telas**, não 3. Vermelho, o que o E97 pagou e ninguém tinha reproduzido: `Error: useBlocker must be used within a data router.` **A conferência, por outro lado, SUBESTIMOU:** trocar de roteador liga um `ErrorBoundary` que não existia, e sem `errorElement` a vendedora leria `<h2>Unexpected Application Error!</h2>` com a pilha num `<pre>` — a classe que o E92 e o E99 pagaram para matar. **E o E2E pegou o que 483 testes de unidade verdes não pegavam:** o bloqueio disparava na navegação do PRÓPRIO save e dois specs caíram — a régua de saída passou a ser uma só (`sujoParaConfirmar` + `liberarSaida`), onde havia cinco grafias e três sítios sem guarda nenhuma. | 🟡 | [E97](execucao/E97.md) · onda 1 de 2026-08-06 |
| ~~S14~~ | ~~**As avarias antigas ficaram sem `parcela_id`.** Não há backfill possível: casar por texto ("Reparo de avaria — …") adivinharia, e duas avarias com a mesma descrição no mesmo contrato são indistinguíveis — que é justamente o caso do duplo clique. Elas seguem cobráveis de novo e removíveis; a guarda vale para o que nasce daqui.~~ **FECHADA POR DECISÃO em 2026-08-06, e a decisão é a que o próprio texto já trazia:** o passivo não se recupera e o aceite está justificado — casar por texto adivinharia, e o caso indistinguível é exatamente o do defeito. Não havia trabalho pendente aqui desde o E97; havia um aceite ocupando linha de tabela como se fosse tarefa. **Se alguém quiser reabrir, o que muda o veredito é uma FONTE nova** (uma coluna de origem na avaria, um registro de auditoria do E97 que amarre avaria a parcela), não um esforço maior de casamento por texto. | 🔵 | [E97](execucao/E97.md) |
| ~~S16~~ | ~~**`leads.contrato_fechado_em` fica `null` em quem tem contrato, e um relatório conta por ela.**~~ O carimbo só é gravado dentro do `if (etapaNova !== lead.etapa)` de `contratos.ts:357` (o carimbo em `:361`) — e `transicaoLeadValida` aceita pular no funil (`iPara > iDe`), então um lead levado de `NOVO` direto a `EM_PROVAS` fecha contrato sem que a etapa mude, e a coluna nunca é preenchida. O `comContrato` de `/leads/sazonalidade` (`leads.ts:397`) filtra por `contratoFechadoEm is not null`: aquela noiva não é contada como "já fechou" na curva que diz quando falta vestido. O conserto é gravar o carimbo mesmo quando a etapa não avança; o backfill pergunta à tabela de contratos, que é a fonte. **CONFERIDA: o mecanismo está VIVO e o relatório está cego por OUTRO motivo — é o achado.** O carimbo segue dentro da guarda (`contratos.ts:579-588`, hoje; a sobra citava `:357/361`), e há uma SEGUNDA porta que ela não viu: `leads.ts:45`, o `carimboEtapa` do PATCH, só carimba quando a etapa é `CONTRATO_FECHADO` exatamente — as duas rotas conspiram. Medido: **792 leads têm contrato e só 3 estão sem carimbo (0,38%), os três inserções diretas de fixture**; nenhum lead em etapa convertida está sem carimbo. Mas a query recorta `casamento_data` nos próximos 12 meses, e **dos 1.279 leads apenas 4 têm `casamento_data`, todas em 2027-12-14 — fora da janela.** `/leads/sazonalidade` devolve **0 linhas** e a tela cai no ramo vazio: a curva que diz quando falta vestido está cega por falta de data de casamento, não por falta de carimbo. O backfill proposto **não foi feito** (nenhuma migração menciona `contrato_fechado_em`). [conferência de 2026-08-05](../2026-08-05-conferencia-de-sobras.md) **FECHADA em `408d0bb`, com o vermelho literal que faltava:** `expected null to be an instance of Date`, num teste que monta o caminho pela API — lead `NOVO`, PATCH para `EM_PROVAS`, `POST /contratos` — e afirma que a etapa não regride E que o carimbo existe. O carimbo saiu de dentro do `if`, e a razão está escrita no lugar: ele é efeito do CONTRATO, não do avanço. A conferência estava certa nas duas portas e no número (836 leads com contrato hoje, 3 sem carimbo, 0,36%, os três em `NOVO` — fixture direta). **O backfill foi feito e rodou** (`docs/migracoes/2026-08-06-s16-carimbo-do-contrato-no-lead.sql`): `UPDATE 3`, e a conferência do próprio script devolve 836 com contrato e **0 sem carimbo**. A fonte é `contratos.fechado_em`, não `created_at` — coincidem em produção, divergem 51 dias na média neste banco. **O que a conferência viu sobre o relatório cego sai daqui inteiro e vira a S39**, porque é outro defeito e cresceu na remedição. | 🟡 | [E98](execucao/E98.md) parte 2 |
| ~~S17~~ | **FECHADA em `14a782a`.** `PATCH /lojas/:lojaId/dados`, gateado por `admin.editar`, e o card em Configurações. **O terceiro dependente que a sobra tinha achado virou o ponto da tela:** o número que não vira link de WhatsApp é recusado com uma frase, e o card avisa enquanto a pessoa digita — antes, telefone errado degradava tão calado quanto telefone vazio. **Duas medições mudaram o código:** o zod gerado ESTRIPA campo desconhecido, então `{ ativo: false }` respondia 200 e não desativava nada (virou 400 `CAMPO_NAO_EDITAVEL`); e corpo vazio estourava no driver com 500 (virou 400 `NADA_PARA_ALTERAR`). `ativo` fica fora de propósito: desativar a própria loja é ato de superadmin. O diagnóstico original: **A dona da loja não consegue editar os dados da própria loja.** `endereco` e `telefone` de `lojas` só têm formulário no console de SUPERADMIN (`pages/admin/index.tsx:560`), que é rota top-level fora do `/loja/:lojaId` com gate próprio (`App.tsx:270`); `/configuracoes` tem backup, captação e privacidade, e nada da loja. **Corrigido pelo mapeamento da fase A:** são DUAS coisas, não três — o rodapé do portal (F35) e a linha "Endereço:" da confirmação (`msgConfirmacaoAtendimento`). O cabeçalho do PDF **não usa** nenhum dos dois: `contrato-pdf.ts` e `contrato-do-papel.ts` têm zero ocorrências de `endereco`/`telefone`, e o papel monta só `lojaNome`. E há um TERCEIRO dependente que ninguém tinha visto: `linkWhatsApp` devolve `null` para telefone fora de 10–13 dígitos, e o botão do portal simplesmente não é renderizado — telefone ERRADO degrada tão calado quanto telefone vazio. Trocar de telefone vira chamado para quem tem o console. | 🟠 | [E100](execucao/E100.md) parte 3 |
| ~~S18~~ | **FECHADA em `4274ad1`, e o conserto é do MECANISMO, não do banco.** `limparFixture` passou a ler os vínculos da loja ANTES de ela sumir e a apagar quem ficou sem vínculo NENHUM — quem trabalha em outra loja fica, porque a fixture não é dona da pessoa, e há teste pregando isso. Medido depois: **1.667 → 42 usuários**, órfãos **1.629 → 4**, e os quatro ficam de propósito (têm história ou são superadmin — o E91 decidiu que apagar quem tem histórico nunca é o caminho). Uma passada completa da suíte não mexe mais nesse número. **Sobrou um caso, e é honesto:** um teste REMOVE o membro da loja de propósito (é o que ele prova), e a partir daí a pessoa não aparece na varredura de vínculos — o arquivo passou a anotar os membros que cria. A rede contra a volta é um teste que reprova se os órfãos passarem de 200, e a limpeza do passivo está em `docs/migracoes/2026-08-05-s18-limpeza-de-usuarios-orfaos.sql`, com três guardas (sem vínculo, sem história em nenhuma das cinco tabelas de autoria, e nunca superadmin). **As 223 atributos e o acervo que esta linha contava foram tratados à parte, na S-A13** (`e2bb58b`); as 186 cabines seguem de pé (S-D25). O diagnóstico original: **REMEDIDA em 2026-08-05, e está PIOR.** Eu a suspeitei morta (o texto abaixo diz "consertado dentro do E100") e a medição desmentiu: o risco da ELEIÇÃO acabou mesmo, mas o passivo que a sobra rediagnosticou cresceu **2,7×** — os usuários órfãos passaram de 613 para **1.629 (98% dos 1.667)**, e há 630 sessões, 24 lojas, 223 atributos e 186 cabines de teste no banco de dev. O mecanismo continua o descrito: `limparFixture` apaga usuário POR ID, e quem nasce da rota `POST /equipe` nunca entra no `Fixture`. O diagnóstico do E100: **O seed do E2E elegia a loja por ordem física de linha, e isso QUEBROU.** Consertado dentro do E100 parte 3 porque a suíte não rodava. **A fase A mediu e o diagnóstico muda de ponta:** o risco da eleição JÁ NÃO EXISTE (o seed elege por `createdAt` desde `global-setup.ts:75`), e as quatro "Loja Teste" são 1,5% do passivo — são **613 usuários órfãos** (86% dos 714) e **723 sessões**. O mecanismo não é "fixture não limpa": `limparFixture` apaga usuário POR ID (`helpers.ts:114`) e 604 dos órfãos nascem da ROTA `POST /equipe`, cujo id nunca entra no `Fixture`. O custo hoje é medível: `GET /admin/usuarios` (`admin.ts:263`) não tem paginação e devolve as 714 linhas. Higiene de fixture em lugar COMPARTILHADO não é higiene: é uma bomba esperando a primeira escrita. Apagá-las continua sendo trabalho do E104. | 🟡 | [E100](execucao/E100.md) parte 3 |
...
| ~~S21~~ | ~~**O "UM pacote" do F34 não foi feito, e a medida diz por quê.** Os quatro exports do financeiro recortam por réguas DIFERENTES — o de parcelas por `vencimento`, o da folha por `data` de pagamento. Juntá-los num arquivo como estão produz um pacote em REGIME MISTO, que não fecha com o DRE nem com o fluxo. Fazer direito é derivá-lo de `GET /financeiro/fluxo`, que já entrega os dois lados com a régua única — é épico de exportação, não uma linha do F34. O que ficou único no F34 foi o CARIMBO.~~ **FECHADA em `d8ef73f`**, derivada do fluxo como a sobra mandava: `GET /financeiro/fluxo/exportar` monta os movimentos pela MESMA função da rota do fluxo (`lib/fluxo.ts`, puro), entradas por recebimento e saídas por pagamento no dia local de SP, rodapé do `resumoCaixa` — fecha com a tela e com o DRE por construção, e o teste prova a fronteira de fuso. A tela já tinha um Exportar client-side (o item estava desatualizado); repontou para o servidor e o gerador do cliente foi aposentado. Os quatro exports antigos ficam: são lentes, não redundância. | 🟡 | [E103/F34](execucao/E103-f34.md) |
| ~~S22~~ | **JÁ ESTAVA FECHADA, e a tabela não sabia — consertada no E115/E146.** O spec restaura `diasFuncionamento` num `finally` (`e2e/24-dias-funcionamento.spec.ts:98-101`), cria cabine própria por execução e a apaga no `afterAll`. Mesma medição das irmãs: quatro passadas completas do E2E hoje, 156/156. O diagnóstico original: **`e2e/24-dias-funcionamento` é flake.** Vermelho numa passada da suíte completa, verde sozinho e verde na execução seguinte. Mesma família da **S20** e da **S7**: um vermelho desses se lê como regressão e custa uma investigação. O spec mexe em `regra_disponibilidade.diasFuncionamento`, que é estado COMPARTILHADO da loja de seed — a suspeita é a mesma classe, e a saída também (configuração própria por execução). | 🟠 (infra de teste) | [E103/F34](execucao/E103-f34.md) |
| ~~S23~~ | ~~**O `mockup-sandbox` guarda 4 cópias divergentes de `ui/`, e uma delas nunca recebeu o conserto do S19.** O `calendar.tsx` de lá continua com as cinco classes `-[--cell-size]` que o Tailwind v4 descarta. Enquanto o pacote estiver fora do workspace (A6) ninguém as compila, então o custo hoje é ZERO — o risco é o dia em que ele voltar, ou em que alguém copiar dali para cá achando que é o primitivo vivo. Ou se apaga o pacote (`rm -rf`, e as 4 cópias vão junto), ou o `ui/` de lá vira um link para o de verdade. **CONFERIDA: "4 cópias" nunca foi verdade, e a premissa do custo zero MORREU.** O repositório tem **2** diretórios `components/ui/` (`git ls-files`): 55 arquivos no mockup, 33 no app; dos 33 homônimos, **8 divergem**. E o `mockup-sandbox` **voltou ao workspace** no E104 parte 4 (`0910ab6`, `pnpm-workspace.yaml:29`) — "enquanto o pacote estiver fora do workspace" não vale mais. O que zera o custo hoje é outra coisa: `src/components/mockups/` está VAZIO e o `.generated/mockup-components.ts` declara 0 módulos, então nada importa o `calendar.tsx`. O CSS morto do S19 está lá: **10 ocorrências em 7 linhas**. Ele deixa de custar zero no primeiro mockup que importar o calendário. [conferência de 2026-08-05](../2026-08-05-conferencia-de-sobras.md) **CONFERIDA na onda 1 de 2026-08-06, e NÃO executada: o `rm -rf` é decisão da dona.** Apagar o `mockup-sandbox` destrói o preview do Canvas que o E104 restaurou (`artifacts/mockup-sandbox/.replit-artifact/artifact.toml:17`). Três medidas novas: (a) **o custo zero tem prova mais forte** que "nada importa o calendário" — o grafo `main.tsx → App.tsx → .generated/mockup-components.ts` para em **0 módulos**, e dos **61** `.ts`/`.tsx` de `src/` o `main.tsx` alcança **3**; (b) **as 8 divergências não são ruído** — cada uma é uma correção do app que a cópia nunca recebeu, e **duas são de acessibilidade** (o `<span>` do badge do E92/E22, o `<h3>` do CardTitle do E136/E12), além do S19 no calendário, do seletor de mês em inglês (E92/D15), dos 44px do botão (E137/E9) e do teto do select (F13/E98); (c) **a segunda saída que a sobra propõe — "o `ui/` de lá vira um link para o de verdade" — está morta:** 22 dos 55 primitivos do mockup não existem no app, e o link apagaria 22 da paleta. O que entrou em `a77e3ef` foi só o número errado que vivia em CÓDIGO: `pnpm-workspace.yaml:27` ainda afirmava "as 4 cópias divergentes de `ui/`", e são 2 diretórios.~~ **FECHADA por DECISÃO em 2026-08-07** (folha de perguntas, respondida na sessão): **o mockup-sandbox fica como está** — o custo de hoje é zero (o grafo alcança 3 dos 61 arquivos) e as 8 divergências ficam aceitas até o primeiro mockup que importar o calendário. | ✅ | [E104](execucao/E104.md) parte 3 · onda 1 de 2026-08-06 · decisão 2026-08-07 |
| ~~S26~~ | **FECHADA em `00a6a66`.** O guard passou a perguntar pelo CARNÊ e não pela existência de parcela — entrou `parcelas.origem` (PLANO | AVULSA | AVARIA), porque a pergunta não era dedutível do dado. **A sobra errou no custo, e a apuração achou a restrição de verdade:** o que a noiva lê é a `descricao`, não o `numero` — mas `numero === 0` significa ENTRADA em **seis** pontos (as três telas de parcela, o portal, a conciliação e o PDF). Deslocar o carnê quebraria a régua nos seis; deslocar o reparo não quebra nada. Vermelho antes: 6 testes, todos `expected 201, got 409`. O diagnóstico original: **Cobrado um reparo, o contrato não consegue mais gerar o carnê.** `contratos.ts:910` recusa `gerar-plano` por `contrato.parcelas.length > 0` — QUALQUER parcela, não "parcela de plano". Cobrar a avaria antes de montar o carnê deixa o contrato em `409 JA_TEM_PLANO` para sempre. Medido depois do E110: `numeroDaAvaria: 1`, `planoStatus: 409`. **O E110 não fechou isto de propósito, e a razão é de escopo:** consertar aquele guard sozinho MOVE a colisão, porque o plano insere `numero` 0..N e o 1 já está tomado pela avaria. Sair disso exige renumerar o carnê ou deslocar o plano — decisão sobre o que a noiva vê no boleto, não uma linha. A saída provável é o guard olhar só as parcelas SEM avaria vinculada (`avarias.parcela_id`), e o plano começar depois do maior número existente. | 🟠 | [E110](execucao/E110.md) |
| ~~S27~~ | ~~**61 `RESERVA_CASAMENTO` no banco de dev não têm `lead_id`** — uma reserva de casamento sem noiva. Descoberto ao desenhar a guarda do E110: `bloqueio_vestidos.lead_id` é nullable, e **61 das 63 avarias** vivem em bloqueio sem noiva, o que obrigou a guarda a "provar quando é provável" em vez de exigir sempre. Falta medir se a ROTA de criação permite `RESERVA_CASAMENTO` sem noiva ou se são resíduo de fixture de API (os helpers de teste aceitam `leadId` opcional) — a resposta muda o achado de "defeito de rota" para "mais um item da S18". Enquanto não se sabe, a guarda do E110 é mais fraca do que poderia ser. **O E111 respondeu metade e encolheu a sobra:** `POST /contratos` passou a ESCREVER `bloqueio.lead_id` (o comentário da guarda S2/E107 já prometia isso — "este contrato é justamente quem lhe dá dono" — e nenhuma linha fazia), então reserva nova nasce com dona e as duas guardas passam a ter o que ler. As 61 legadas continuam sem: para elas quem segura é o 409 `RESERVA_JA_CONTRATADA` do vínculo, não a comparação de noiva. Falta o backfill e a resposta sobre a ROTA de criação. **CONFERIDA em 2026-08-05, e a pergunta em aberto tem resposta: são AS DUAS.** (1) A rota permite — `reservas.ts:336-368` valida vestido e exige `casamentoData`, e nunca exige `leadId` (`api.ts:4454`, `leadId` opcional): reserva de casamento sem noiva é 201 hoje. (2) E são resíduo de fixture — 120 das 124 vivem na loja do seed, 9 criadas hoje. **O número DOBROU: 61 → 124, e as avarias em bloqueio sem noiva de 61 para 120.** O conserto do E111 só alcança bloqueio que um contrato NOVO amarra (`contratos.ts:542-549`), e as 124 legadas não têm contrato. **De brinde, documentação que mente por fator 2 dentro do código:** o comentário da guarda em `reservas.ts:763` ainda afirma "61 das 63 avarias". [conferência de 2026-08-05](../2026-08-05-conferencia-de-sobras.md) **O DEFEITO DE PRODUTO fechou em `3b71a43`; o que resta é decisão de dono, e o peso NÃO subiu — a premissa caiu.** O número era **131**, não 61 nem 124, e **97% vinha do spec 48**, não da rota: a rota permite criar reserva sem noiva e produziu ZERO das 131. O defeito real era outro e está fechado — `DELETE /lojas/:lojaId/leads/:leadId` (`leads.ts:599`) contava contrato, orçamento e atendimento e **nunca contava bloqueio**, então a noiva saía com 204, o vestido ficava ocupado e o único ponteiro para a dona era apagado, contra a promessa do docblock da própria rota; agora responde 409 `LEAD_COM_RESERVA`. **A preparação propôs subir para 🟠 e um CHECK `lead_id IS NOT NULL`, e os dois caíram contra evidência versionada:** o CHECK foi aplicado, a suíte da API devolveu 17 vermelhos, e um deles não era fixture — `e107-prova-e-rastro-api.test.ts:84`, cujo nome é a decisão inteira, **reserva SEM dona é aceita, é o contrato que lhe dá dono**. O E107 estabelece que a loja segura a peça antes de a noiva ter ficha. O dossiê afirmou que "nenhuma tela oferece o contrário" e não viu este teste. **Fica aberto só isto, e é pergunta para a dona:** a reserva sem dona deve continuar possível pela rota, como o E107 decidiu, ou virou sobra do tempo em que não havia ficha rápida? Sem a resposta não há CHECK a aplicar.~~ **FECHADA por DECISÃO em 2026-08-07** (folha de perguntas, respondida na sessão): **reserva sem dona é aceita — é o contrato que lhe dá dono.** A decisão que o teste já tinha por nome vira resposta escrita; acontece de separar peça antes da ficha, e o sistema está certo como está. | ✅ | [E110](execucao/E110.md) · [E111](execucao/E111.md) vp1 · onda 2 de 2026-08-06 · decisão 2026-08-07 |
| ~~S25~~ | ~~**`e2e/48-avaria-vira-parcela` vaza a corrente do vestido a cada execução.** O `afterAll` (`e2e/48:67`) limpa parcelas → contrato → lead, e **não** limpa vestido, bloqueio nem avaria. Medido em duas execuções seguidas hoje: as avarias foram de **62 para 63**, e cada passada acrescenta um vestido `AVA-<stamp>` ao acervo — parte dos 533 vestidos e das 63 avarias do banco de dev sai daqui. Família da **S18**, e a mesma lição: higiene de fixture pela metade não é higiene. **Não é o caso da S7/S20:** não causa flake, porque cada execução usa `stamp` próprio — o custo é o acervo virar lixo e nenhuma tela de vestidos ser avaliável. **REMEDIDA em 2026-08-05, com a S-A13: as avarias passaram de 63 para 121, e os 121 vestidos `AVA…` do banco são exatamente elas** — dobrou em dez dias. A faxina da S-A13 (`e2bb58b`) NÃO os alcança de propósito: eles têm bloqueio e avaria, e a guarda dela só apaga peça sem referência nenhuma. Enquanto o spec não limpar o que cria, faxina nova não adianta. **E esta sobra tem dois nomes em dois rastreadores — é a S-D22 da rodada 7 —, que é a armadilha da S-D28/S-A5:** quem fechar uma precisa riscar a outra.~~ **FECHADA em `3b71a43`, junto com a irmã da outra trilha e com a S27 — as três eram a MESMA linha.** O `afterAll` que a sobra diz faltar **existe desde `5a1e038`** e apagava na ORDEM errada: o lead saía primeiro e, como `bloqueio_vestidos.lead_id` é `SET NULL`, a reserva ficava órfã em vez de sair. Agora o vestido sai antes, e a cascata leva bloqueio e avaria. Medido antes: **131 órfãs, 129 vestidos `AVA-` num acervo de 511 (25%), 127 avarias somando R$ 44.450,00**, nenhuma cobrada — e a taxa de +1 por passada foi medida DENTRO da sessão (130 → 131 entre a leitura do dossiê e a migração). Depois da faxina e de uma passada COMPLETA do E2E: **0 órfãs, 0 `AVA-`, 0 avarias**. A guarda da faxina admitiu **129 de 129** no ensaio em leitura pura, e as 2 noivas `E2E Avaria` ficaram de pé porque cada uma tem contrato — contrato é história, não fixture. | 🟡 (infra de teste) | [E104](execucao/E104.md) parte 4 · onda 2 de 2026-08-06 |
| ~~S30~~ | ~~**Quinze formatadores `Intl` vivem fora da régua, e ninguém verificou um a um se cada um se justifica.** O E92 consolidou 36 → 17 e parou ali de propósito; o E111 apagou mais cinco. A varredura mecânica (`varredura-reguas.test.ts`) travou o número: arquivo NOVO declarando formatador reprova o teste. O que fica aberto é o passivo — `reservas/helpers.ts` sozinho tem seis, e alguns podem ser opção-set que a régua não cobre (mês sozinho, mês no fuso da loja). Consolidar exige decidir quais viram função pública em `formatos.ts`, e isso é julgamento de API de tela, não limpeza. A lista está no teste declarada como **estado de hoje, não atestado**. **CONFERIDA: a afirmação central da sobra está ERRADA.** `varredura-reguas.test.ts:164-186` não trava número nenhum — trava a **lista de 9 arquivos** herdados e afirma que cada um declara *pelo menos um* formatador. Um arquivo herdado pode ir de 1 para 20 com a suíte verde. O passivo é **17, não quinze**, e não cresceu desde `fc2182a` — mas não foi a sonda que impediu. Mesma família da S-D7. [conferência de 2026-08-05](../2026-08-05-conferencia-de-sobras.md) **O MECANISMO fechou em `973c364`; o passivo segue aberto e é o que resta desta linha.** `HERDADOS` virou `Record<string, number>` com a contagem congelada arquivo a arquivo (cobrança 1, whatsapp 2, fluxo 2, projeção 2, minha-comissão 1, noiva-portal 1, conversão 1, noivas/helpers 1, reservas/helpers 6), mais um `it` travando o total. Vermelho literal: um `new Intl.NumberFormat` a mais em `reservas/helpers.ts` (6 → 7) deixava a sonda de HEAD verde com 6 testes; a nova reprova com `expected 18 to be 17`. **Os 17 são os mesmos de `fc2182a` — nada foi destravado.** O que continua aberto é a consolidação das 17 cópias, uma a uma, que é o que a sobra sempre pediu.~~ **FECHADA em `d8ef73f`, com o veredito dos 17 escrito**: 5 apagados (cópias opção a opção de régua pública), 7 promovidos a `formatos.ts` (os pares de fuso que faltavam, com teste na fronteira), **5 ficam com o porquê no arquivo** — weekday por extenso é voz de mensagem, "jul. de 26" é rótulo de eixo. Passivo 17 → 5, vermelho literal da sonda na transição: `expected 5 to be 17`. O objetivo nunca foi zerar: era cada linha ter sido julgada. | 🔵 | [E111](execucao/E111.md) adendo · onda 1 de 2026-08-06 |
| ~~S37~~ | **FECHADA em `85d5108`, e o conserto foi matar a terceira régua em vez de consertá-la.** `whatsappDigits` saiu de `pages/noivas/helpers.ts` e a ficha passou a chamar `linkWhatsApp`: número implausível vira null e o link não é renderizado, comportamento que a tela já sabia tratar (o ramo do `<span>` sem link já existia). **A rede é uma varredura, não um teste do caso** — nenhum arquivo fora de `lib/whatsapp.ts` pode escrever a URL do wa.me, e ela lê o arquivo INTEIRO, nunca linha a linha, porque a S-D7 mostrou no mesmo dia o que varredura por linha custa. Vermelho antes, com o código de ontem: `expected [ 'pages/noivas/[leadId]/index.tsx' ] to deeply equal []`. **O RESIDUAL, e é o que a sobra pedia originalmente:** as duas cópias que CONCORDAM (cliente e servidor) seguem sendo duas — unificá-las pede pacote compartilhado novo, e não há defeito medido que o justifique hoje. Frontend 469 → 473 · E2E 156. O diagnóstico original: **A régua do telefone que vira link de WhatsApp está em DOIS lugares.** `moscow-noivas/src/lib/whatsapp.ts:13` (que decide se o botão do portal existe) e `api-server/src/routes/equipe.ts` (que recusa o cadastro), em pacotes que não se importam — a mesma classe da S11, e nasceu na S17 por não haver lib compartilhada entre servidor e tela para uma régua de CONTATO. `financeiro-core` é o único pacote comum e é o lugar errado. Enquanto viverem separadas, `telefone-da-loja.test.ts` fixa as bordas para as duas: mexeu numa, confira a outra. **CONFERIDA e SOBE PARA 🟠: as duas cópias que a sobra cita CONCORDAM (varredura de 0 a 16 dígitos, zero divergências) — e existe uma TERCEIRA que já entrega link quebrado.** `noivas/helpers.ts:57-61` (`whatsappDigits`) devolve qualquer quantidade de dígitos, sem DDI e sem faixa, e `noivas/[leadId]/index.tsx:464` monta `https://wa.me/${digits}` direto com ela. As **3 noivas com WhatsApp no banco têm 10–11 dígitos e nenhuma tem 55**: o botão da ficha manda `wa.me/11988887777`, que o WhatsApp lê como DDI 1 (EUA), enquanto `mensagens/index.tsx:315` monta o link certo para o MESMO campo. O teste de bordas fixa as duas que já concordam e não toca na que diverge. [conferência de 2026-08-05](../2026-08-05-conferencia-de-sobras.md) | 🔵 (residual) | execução S17 · `85d5108` |
| ~~S36~~ | **FECHADA em `2ee6bf0`, e a varredura achou defeito na primeira execução.** Dois, e o primeiro estava VIVO num perfil PADRÃO: `atendimentos/novo.tsx` gateava por `agenda` e chamava `POST /bloqueios`, que é `vestidos` — a **Recepção** (`agenda: TUDO`, `vestidos: SO_VER`) via o botão "Criar reserva" e levava 403. O segundo, `comissoes/index.tsx`, gateava por `admin` três ações que o servidor guarda por `comissao`; nos quatro perfis padrão ninguém tropeça, então ele esperava a primeira loja que customizasse um perfil. A varredura cruza três declarações que já existiam e ninguém tinha juntado — o `requireModulo` de cada prefixo, a URL de cada operação no cliente gerado, e o `podeNoModulo` de cada tela. O diagnóstico original: **O gate da tela e o guard do servidor podem pedir MÓDULOS diferentes, e nada compara os dois.** Foi o defeito do E111: `cobranca.tsx` e `receber.tsx` gateavam botões por `financeiro.editar` enquanto o servidor guarda a ação por `leads` — quem tinha o acesso certo via a tela sem o botão, e quem não tinha via o botão e levava 403. **Teste de componente não pega esta classe** (ele prega o que a tela faz, não se ela pede o módulo certo) e E2E só pega com perfil customizado, porque nos quatro perfis padrão quem chega ao financeiro tem tudo. O que pega é uma VARREDURA que case o `podeNoModulo(…)` de cada tela com o `requireModulo(…)` da rota que aquele botão chama — a família da `e101-acao-da-rota-api.test.ts`, que já faz isso do lado do servidor. Enquanto ela não existe, gate de tela é conferido a olho. | 🟠 | execução S15 |
| ~~S28~~ | **FECHADA em `6df7df1`.** A varredura existe e roda sobre toda a suíte (API, frontend e E2E). **Ela pegou uma lição no meio:** a versão só de `expect(x).toBe(x)` passou de primeira, e foi buscar o arquivo como estava antes do E111 (`58ea660^`) que mostrou que ela NÃO pegava o defeito original — lá os dois lados eram textualmente diferentes, e a tautologia estava um passo atrás, nas duas variáveis nascidas da mesma expressão. A segunda forma quase não é detectável por texto: sem refinamento acusou **7 testes, os 7 legítimos** (cinco leituras ANTES/DEPOIS, dois nomes repetidos em `it`s diferentes). O que separa o defeito do sósia é o código no meio — com a régua de adjacência, ela pega o caso histórico e não acusa nenhum dos sete. O diagnóstico original: **Uma sonda do E2E provava a si mesma, e nada garante que fosse a única.** `e2e/08-contratos.spec.ts:56` comparava a MESMA string literal consigo mesma — o assert media o status de uma requisição contra o dele próprio e passava sempre, inclusive se as duas dessem 404. Consertada no E111 (a URL passou a sair de `getGetContratoUrl`, do cliente gerado). O que fica aberto é a **varredura**: a mesma classe de defeito cabe em qualquer `expect(x).toBe(x)` e em qualquer assert cujos dois lados nasçam da mesma expressão. O E101 já provou que uma lista para de crescer sozinha; um teste tautológico é pior, porque ele conta como cobertura. | 🟠 (infra de teste) | [E111](execucao/E111.md) vp3 |
| ~~S29~~ | ~~**A raiz do workspace ganhou `@workspace/api-client-react` só para o E2E.** A sonda do S28 só testa algo se a URL vier do cliente GERADO, e importá-lo da raiz exigiu a devDependency. É decisão registrada, não descuido: sem o import, a sonda volta a repetir uma literal que o codegen pode invalidar sem ninguém notar. Se o dono preferir não pagar a dependência na raiz, o caminho alternativo é a sonda afirmar só o `200` na URL literal — mata a tautologia, perde a amarração com o codegen. **CONFERIDA na onda 1 de 2026-08-06: não há o que podar, e a pergunta é de dono.** `package.json:20` existe para **1 import em 1 spec** (`e2e/08-contratos.spec.ts:4`), e `workspace:*` é symlink — **0 bytes de rede**. O outro arquivo que a varredura devolve (`s36-gate-da-tela-unit.test.ts:50`) lê o cliente por CAMINHO e não usa a dependência, então a raiz paga por um consumidor só. Fica como está até a dona decidir entre manter a amarração com o codegen e a sonda afirmar só o `200` na URL literal.~~ **FECHADA por DECISÃO em 2026-08-07** (folha de perguntas, respondida na sessão): **a dependência fica na raiz** — é a amarração da sonda com a URL que o codegen gera, e custa 0 bytes de rede. | ✅ | [E111](execucao/E111.md) vp4 · onda 1 de 2026-08-06 · decisão 2026-08-07 |
| ~~S31~~ | ~~**`lead_interesse_atributos.atributo_id` não tem cascade.** A cascata da loja tenta apagar o atributo antes das linhas do interesse e leva 23503; apagar um atributo com interesse de noiva referenciando dá 500 cru pela mesma FK. Descoberto pelo teardown do teste de fronteira do E115.~~ **FECHADA em `0e395ae`, e a sobra errava as duas metades.** São **4 FKs** sem cascade, não uma, e as quatro nasceram em `NO ACTION` por OMISSÃO — sem cláusula `ON DELETE` nenhuma. **A quinta da família (`atributo_opcoes_atributo_id`) JÁ era CASCADE, e é a assimetria que produz o defeito:** a cascata da loja apaga `atributos`, o gatilho das opções cascateia, e as checagens NO ACTION encontram linhas que só sumiriam dezenas de posições depois. **E não dava 500:** o Express 5 traduz o `23503` em 409 `VINCULO_EXISTENTE` — recusa plausível para um pedido correto, sem dizer o que está preso. As quatro passaram a CASCADE pela régua do E91 (configuração cascateia, história recusa: o que a noiva escreveu com as próprias palavras mora em `lead_interesses` e fica), e a guarda entrou na ROTA — 409 `ATRIBUTO_EM_USO` dizendo quantas peças e quantas noivas dependiam da palavra. De passagem, defeito: `DELETE /atributos/opcoes/:id` respondia **204 para opção inexistente**. O épico recolheu os contornos que a falta de cascade obrigava — o `afterAll` do `e115-fronteira-corpo` e as duas frases "sem cascade" dos specs 27 e 30, que passaram a mentir (regra 21).  | 🟡 | [E115](execucao/E115.md) vp1 · onda 2 de 2026-08-06 |
| ~~S32~~ | ~~**`requireSessaoComLoja` roda até 11 vezes por request** (22 consultas sequenciais de sessão+loja): os dez routers de domínio o montam em série e o Express atravessa todos até casar a rota. É o custo do conserto do E111, no caminho mais quente do servidor — medido pela revisão do E115 e fora do corte de severidade. **CONFERIDA, e agora MEDIDA em vez de deduzida.** Os dez routers que montam o guard sem path estão confirmados um a um (`equipe:31`, `catalogo:26`, `vestidos:62`, `leads:72`, `agenda:178`, `reservas:44`, `orcamentos:114`, `contratos:57`, `financeiro:108`, `comissao:58`), e o 11º é o próprio `dashboard.ts:27`. Contagem real num Express 5 com a forma exata do `index.ts`: **`GET /dashboard` roda o guard 11× = 22 consultas sequenciais; `comissao` 20; `financeiro` 18; `equipe` 2.** O custo é função da POSIÇÃO no `index.ts` — a tela de abertura é a que paga mais caro, e qualquer router novo montado sem path antes dela a leva a 24. Os `requireModulo` não entram na conta (todos montados com path). [conferência de 2026-08-05](../2026-08-05-conferencia-de-sobras.md)~~ **FECHADA em `f901275`**: memoização POR REQUEST no próprio guard — quem já foi autenticado por uma execução anterior dele na mesma request pula direto para a checagem de loja da URL, e o custo deixa de ser função da posição no `index.ts`. Medido no dev server: mediana do `GET /dashboard` de **15,3 ms para 5,3 ms (−65%)**. O teste prova o mecanismo por construção (req sem cookie no ramo memoizado passaria por 401 sem a memo), inclusive o caso "requireSessao sozinho não arma o atalho" — `lojaAtiva` só nasce no guard cheio. | 🟡 | [E115](execucao/E115.md) vp2 |
| ~~S33~~ | ~~**A guarda do E106 no `DELETE /admin/lojas` conta e apaga fora de transação** — entre a contagem do histórico e o delete cabe uma escrita. O verificador marcou PLAUSIBLE (mecanismo real, corrida não reproduzida). **CONFERIDA: a sobra descreve errado, e o conserto que ela sugere não fecha a corrida.** O `DELETE` **está** em transação (`admin.ts:201-211`); quem fica de fora é a **leitura da guarda** (`:165-172`). Mover a contagem para dentro do mesmo `tx` não basta: o servidor roda em READ COMMITTED (grep por `for update|isolationLevel|serializable` = **0 ocorrências**), onde a linha inserida por outra sessão continua invisível para o `SELECT count()` e visível para o `DELETE`. Precisa de `FOR UPDATE`, `SERIALIZABLE` ou tirar o CASCADE. **E é pior: são 33 FKs em CASCADE para `lojas` e ZERO `restrict`** (o comentário de `:126` diz 31) — não há rede do banco. O irmão `DELETE /admin/usuarios` tem a forma idêntica e não corre o risco, porque lá existem 5 `restrict` que abortam a corrida. [conferência de 2026-08-05](../2026-08-05-conferencia-de-sobras.md)~~ **FECHADA em `24e9054`**, pela terceira via que a conferência listou: `SELECT ... FOR UPDATE` na linha da loja, dentro da transação que agora envolve leitura E delete — KEY SHARE de INSERT filho conflita com FOR UPDATE, então o insert em voo segura a tranca e a contagem (statement novo) o vê. **A corrida foi reproduzida de verdade num teste com duas conexões** (INSERT não commitado + rota pendurada na tranca), com o vermelho literal contra o código velho: `expected 204 to be 409` — o 204 levava o lead recém-nascido no cascade. As FKs foram remedidas: **33 CASCADE, 0 RESTRICT, 1 SET NULL** (o comentário dizia 31). | 🔵 | [E115](execucao/E115.md) vp3 |
| ~~S34~~ | ~~**`GET /pagamentos` devolve 400 com FRASE no campo `error`** ("Filtro inválido (de/ate esperam YYYY-MM-DD)") — a família da S12, que o E107 tinha zerado, tem uma sobrevivente; as oito rotas irmãs emitem código.~~ **FECHADA em `4ea4fe2`.** A **S34 e a S-D21 são a MESMA linha** (`financeiro.ts:445`) em duas trilhas — terceira vez neste repositório, depois de S-D28/S-A5 e S25/S-D22. **A S-D21 lista 7 sites e existem 21**, medidos um a um: 18 por `res.status(4xx).json(` e 3 nos corpos `message` dos rate-limiters, que não passam por `res` nenhum; quatro das sete âncoras dela já não apontavam para a linha certa. **Os 13 que nenhuma das três viu são os que mais rodam** — 8 em `middlewares/auth.ts`, no caminho de toda requisição autenticada. A varredura do E145 parava em `routes/` e passou a varrer `src/`: **414 sites em 19 arquivos**, contra 117 em 20 rotas. **O custo que nenhuma mediu:** as quatro páginas públicas leem `data.error` como CHAVE, então a noiva que esbarrava no teto de requisições lia *"Link inválido — confira se ele veio inteiro do WhatsApp"* sobre um link perfeito — e **nenhuma das 1.038 provas de API nem dos 161 specs podia ver**, porque os três limitadores são pulados sob `VITEST` e `E2E_SUITE`. | 🔵 | [E115](execucao/E115.md) vp4 · onda 2 de 2026-08-06 |
| ~~S35~~ | ~~**18 achados verificados da revisão E115 abaixo do corte, não aplicados**: duplicações (`competenciaValida`, três `-3h` à mão na comissão, `haQuanto` ×3 com tetos divergentes, `isoParaDia`, `normalizar` ×2, envelope CSV ×4, parse `de`/`ate` ×9, janela da folha inline), código morto (`compararSenha`, `useIsMobile`, `prazoCasamento`), N+1 (bloqueios do PATCH de contrato, simulador de comissão) e o dashboard executando consultas de dinheiro que descarta. Mais dois vistos de passagem: `pagar.tsx` sem gate de ação e os POSTs antigos da agenda respondendo 404 (não 422) a id de corpo inválido. Lista completa com âncoras no adendo do E115.~~ **FECHADA em `cafe56c`**, com equivalência PROVADA antes de cada troca: 100 meses enumerados na competenciaValida, **30.750 instantes** na varredura dos `-3h` (que corrigiu a premissa do repo — "sem DST desde 2019" tem fronteira em 17/02/2019), comparação literal no isoParaDia, BOM pregado por `charCodeAt` no envelope CSV, e as 9 cópias de `de`/`ate` unificadas sem mudar uma mensagem. N+1 medidos: PATCH de contrato 5 → 2 consultas, simulador 11 → 5, dashboard sem dinheiro 7 → 5. Os residuais viraram **S40** e **S41**. | 🟡 | [E115](execucao/E115.md) adendo |
| ~~S38~~ | ~~**Dois worktrees de agente saíram do git e continuaram no disco: 65% do que as varreduras liam era cópia deles.**~~ **Nasceu e fechou em 2026-08-06**, na higiene que abriu a onda da faixa B. `git worktree list` mostrava só o `main`, `git status` estava limpo e `.git/worktrees/` nem existia — a poda do git tinha rodado e **não levou os arquivos**: `.claude/worktrees/agent-a6159143e2c332023` (527 MB) e `agent-ad34374f849651b49` (511 MB), **1,04 GB**, invisíveis ao git por `.git/info/exclude:11` (`**/.claude/worktrees/`) e visíveis a todo `find` e `grep -r`. Medido antes e depois de apagar: **arquivos `.ts`/`.tsx` 4.791 → 1.681**, `const` declarados **23.845 → 8.970**, formatadores `Intl` **82 → 25**, consumidores de `useIsMobile` **10 → 3**, diretórios `components/ui` **12 → 5**. Ou seja: uma varredura ingênua da S30 teria contado **82 formatadores** onde há 25 (a conferência mediu 17 na régua certa), e a S-D6 — cuja tese é "0 consumidores" — devolvia 10 arquivos, 7 deles cópia. **Nada se perdeu:** o órfão `ad34…` tinha **zero** arquivos só dele, e os 30 do `a615…` fora do `.migration-backup/` foram todos apagados do `main` por épico com nome (`dinheiro.ts` e `parcelas.ts` no E95 `c4d8609`; `estado-erro.tsx` e 23 primitivos de `ui/` no E99 `c8ff967`; `scripts/src/hello.ts` no E111 `58ea660`; a migração `0000_perpetual_warstar.sql` no E115 `9a2f4ca`). **O conserto não tem diff** — os diretórios estavam fora do versionamento, e este registro é o único rastro que ele deixou. | 🟡 | sessão de 2026-08-06 |
| ~~S24~~ | ~~**A allowlist do `lote2` não cobra que cada perdão ainda tenha assunto** — foi assim que o `DELETE /ajustes/{ajusteId}` seguiu perdoado por tempo indeterminado depois de já estar no spec (`openapi.yaml:1767`). A guarda **não** foi escrita no E104 parte 3 porque o conjunto ficou VAZIO e ela não teria sujeito.~~ **FECHADA POR DECISÃO em 2026-08-06: não é trabalho, é uma REGRA condicional, e ela fica escrita onde vai ser lida.** A allowlist está vazia e uma guarda sem sujeito não guarda nada — a S-D31 mediu no mesmo repositório o que custa uma varredura sobre conjunto vazio: ela aprova tudo em silêncio. **A regra que sobra:** *toda entrada nova na allowlist do `lote2` entra junto com a guarda que a cobra — no servidor E ausente do spec —, no mesmo commit.* Enquanto a lista tiver zero entradas, não há o que fazer, e a linha só obrigava quem lesse a tabela a redescobrir isso. | 🔵 | [E104](execucao/E104.md) parte 3 |
| ~~S39~~ | ~~**A curva de sazonalidade nunca foi desenhada com dado, e ninguém tem como saber se ela funciona.** `/leads/sazonalidade` (`leads.ts:444-462`) recorta `casamento_data` entre agora e 12 meses; medido em `psql`: **4 de 1.351 leads têm a data**, as quatro em `2027-12-14` e nenhuma na janela, então a rota devolve **0 linhas para TODA loja deste banco** e `noivas/conversao.tsx:243` cai no ramo *"Nenhum casamento com data marcada nos próximos meses"*. A barra que a S16 alimenta — "a parte cheia da barra já tem contrato" — **nunca renderizou**. E a cegueira é de dado, não de código: `contratos.data_casamento` está em **0 de 836**; onde a data aparece de fato é `bloqueio_vestidos.casamento_data`, **120 de 220 linhas**, mas em só **5 noivas distintas** (o resto são bloqueios sem noiva, a família da S27). **Nada reclama disso em nenhuma direção:** não há captura de `/noivas/conversao` entre as 94 da rodada 7, e o único spec que abre a tela (`20-conversao-leads.spec.ts:15`) afirma "Por origem" e "Por que se perderam" e não olha o cartão — passa igual com a curva vazia. As duas perguntas são para a dona: o ateliê registra a data do casamento em algum lugar, e se registra, por que ela não chega ao lead?~~ **FECHADA por DECISÃO em 2026-08-07** (folha de perguntas, respondida na sessão): **a dona ANOTA a data do casamento — ela passa a ser digitada no cadastro que já existe.** O buraco é de uso, não de código: nenhuma mudança de sistema; a curva 'quando faltará vestido' ganha matéria-prima conforme as fichas forem preenchidas, e o dia em que houver dado ela já desenha. | ✅ | S16 · remedida em 2026-08-06 · decisão 2026-08-07 |
| ~~S40~~ | ~~**`pagar.tsx` não tem gate de ação.** Remedido no fecho da S35: zero `podeNoModulo` no arquivo; "Registrar pagamento" (`pagar.tsx:736`) e "Lançar" (`:815`) só desabilitam por `isPending`. O servidor recusa por trás (o gate do endpoint vale), mas a tela oferece o botão a quem não pode — o padrão para copiar está vivo em `receber.tsx:91` (`podeMovimentar`). É a tela de DINHEIRO saindo.~~ **FECHADA em `475b991`, e a remedição achou mais do que a sobra dizia:** não eram dois botões, eram **seis pontos de oferta** — Lançar despesa (`:418`), Pagar por linha e por seleção, Remover, Estornar, mais os checkboxes que só existem para pagar. Todos atrás do gate que o servidor cobra (criar para os POSTs, editar para o DELETE). E a varredura S36 era cega para o caso por desenho (`gates.size === 0 → continue`): ganhou o assert "toda tela que ESCREVE em financeiro afirma um gate" — vermelho antes com `pagar.tsx` literal. De passagem nasceram a S42 (a última tela sem gate) e a S43 (o estorno exige `criar`). | ✅ | E115 vp · S35 · `475b991` |
| ~~S41~~ | ~~**Dois residuais do E115 que a S35 mediu e deixou de propósito.** (a) Os POSTs antigos da agenda respondem **404 `REFERENCIA_INVALIDA`** a id de corpo inválido (`agenda.ts:296`, `:365`) onde a régua do E91 pede 422 — o guard da era E115 no mesmo arquivo (`:755`) já responde 422 para o mesmo caso. (b) O laço do **POST /contratos** (`contratos.ts:311`) é o irmão N+1 do PATCH que a S35 consertou, mas tem 4 erros com precedência por bloqueio e `verificarDisponibilidade` por candidato — consolidar muda contagem de consultas em caminhos de erro, e o risco não coube no escopo.~~ **FECHADA em `f0f1f49`, e a remedição triplicou a metade (a):** a régua do E91 estava furada em **seis** sítios, não dois — os dois da agenda, o `POST /ausencias` e três em `reservas.ts` (`:78`, `:354`, `:358`). Sete asserts pregavam o 404 e viraram 422 com os títulos junto. A metade (b): os DOIS SELECTs por bloqueio do `POST /contratos` viraram duas consultas com `inArray` fora do laço (2N+N → 2+N para N reservas); o laço e a precedência dos quatro erros ficam, porque é ela que os testes pregam, e `verificarDisponibilidade` segue por candidato de propósito. E2E completo verde depois: 161/161. | ✅ | E115 vp · S35 (`cafe56c`) · `f0f1f49` |
| ~~S42~~ | ~~**`conciliacao.tsx` chama `useMarcarConciliado` com zero `podeNoModulo`** — a mesma classe da S40, medida no fecho dela: das 9 telas de `pages/financeiro/`, era a ÚNICA restante que escreve (POST `/financeiro/conciliacao/marcar`, guardado por `financeiro`/criar via prefixo `financeiro.ts:111`) sem afirmar gate nenhum. A exposição é menor que a da S40 (marcar conciliado é idempotente e não move dinheiro — o `WHERE` exige `conciliadoEm IS NULL`), por isso ficou fora do escopo. A régua nova da S40 (`s36-gate-da-tela-unit.test.ts`, "toda tela que ESCREVE em financeiro afirma um gate") já a vigia por exceção anotada: quem fechar esta linha apaga a entrada de `DIVIDA_ANOTADA` e o teste passa a cobri-la.~~ **FECHADA em `05cf366`.** O único ponto que escreve ("Marcar N como conferidas") passou a exigir financeiro/criar; a comparação com o extrato segue aberta a quem só vê, e o ramo "todas já conferidas" não substitui o botão escondido — com casadas novas na tela ele seria mentira. A `DIVIDA_ANOTADA` da régua S36 esvaziou: vermelho antes literal com a exceção removida. Das 9 telas de financeiro, as 5 que escrevem afirmam gate. | ✅ | S40 vp · `05cf366` |
| ~~S43~~ | ~~**A ação exigida para ESTORNAR um pagamento é `criar`, e a assimetria é de dinheiro.** O middleware deriva a ação do método (`auth.ts:125`: POST→criar, DELETE→editar) e `POST /financeiro/pagamentos/:id/estornar` (`financeiro.ts:518`) não tem override — perfil customizado com `financeiro: {criar}` sem `editar` **desfaz rastro de caixa** mas não pode remover uma conta PREVISTA (DELETE→editar). E as duas rotas que pagam conta divergem entre si: `/contas-pagar/:id/pagar` exige `editar` explícito (`financeiro.ts:330`) e `POST /financeiro/pagamentos` — a única com consumidor de tela — deriva `criar`. Compare: o estorno de RECEBIMENTO tem gate explícito (`contratos.ts:76`, leads/editar). Nos 4 perfis padrão não muda nada (`financeiro` é TUDO ou NADA — `configuracao-inicial.ts:90-108`); a fresta é só de perfil customizado. Decidir a ação certa é regra 5 (pergunta antes de código); a tela da S40 espelha o servidor de hoje (`podeCriar` no estorno) e acompanha quando ele mudar.~~ **FECHADA por DECISÃO em 2026-08-07** (folha de perguntas, respondida na sessão): **o estorno com permissão de `criar` fica como está** — a fresta é só de perfil customizado (nos 4 perfis padrão `financeiro` é TUDO ou NADA), e fica escrito que criar basta para estornar. A tela de pagar já espelha exatamente isso desde `475b991`. | ✅ | S40 vp · decisão 2026-08-07 |
...
| ~~E104~~ | ~~**O `.migration-backup/` saiu do versionamento e continua no DISCO.** O A4 resolveu o repo, não a busca: `find`/`grep` no workspace ainda devolvem dois resultados para cada arquivo vivo (aconteceu duas vezes na sessão 4, procurando `openapi.yaml` e `schema.ts`). Apagar é uma linha; o que falta é a decisão de apagar.~~ **Fechada em 2026-08-06 pela higiene da S38**, e por um motivo que ninguém previu: na raiz ele já não existia — o `.migration-backup/` sobrevivia **dentro de um worktree órfão**, com 1.225 arquivos só dele. Apagar o órfão levou os dois. A "decisão de apagar" que faltava era sobre um diretório que tinha mudado de lugar. | [E98](execucao/E98.md) parte 2 vp2 · [S38](#sobras--visto-de-passagem-sem-épico) |
