...
describe("indicação de vestido por interesse", () => {
  it("ranqueia por afinidade, com orçamento como desempate suave, e ignora 0 pontos", async () => {
    const r = await indicarVestidos(loja, lead);
    // Dentro-do-teto vem primeiro; W (0 pts) some.
    expect(r.map((v) => v.nome)).toEqual(["Perfeito", "Meio", "Caro"]);
    expect(r[0]).toMatchObject({ pontos: 2, total: 2, dentroDoOrcamento: true });
    expect(r[0].combinam.sort()).toEqual(["Decote", "Tipo de saia"]);
